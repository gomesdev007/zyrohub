...
--==================================================
-- EGG ESP (integrated; standalone AutoSteal code intentionally ignored)
--==================================================
local EggESPEnabled = false
local EggESPFolder = nil
local EggESPTags = {}
local EggESPUpdate = 1.2

local EggESPNetworking = game:GetService("ReplicatedStorage"):FindFirstChild("Networking")
local EggSnapshotESP = EggESPNetworking and EggESPNetworking:FindFirstChild("EggSnapshot")

local EggESPRarityColors = {
...
local function getEggESPContainer(uid)
    local folder = workspace:FindFirstChild("AreaEggSlotsClient")
    if not folder then return nil end
    for _, child in ipairs(folder:GetDescendants()) do
        if child.Name == tostring(uid) then return child end
    end
    return nil
end

local function getEggESPPart(uid)
    local container = getEggESPContainer(uid)
    if not container then return nil end
    if container:IsA("BasePart") then return container end
    return container.PrimaryPart
        or container:FindFirstChild("Hitbox")
        or container:FindFirstChildWhichIsA("BasePart")
end

local function getEggESPModel(uid)
    local container = getEggESPContainer(uid)
    if not container then return nil end
    if container:IsA("Model") then return container end
    return container:FindFirstAncestorOfClass("Model") or container
end

local function getEggESPFolder()
    if EggESPFolder and EggESPFolder.Parent then return EggESPFolder end
    EggESPFolder = Instance.new("Folder")
    EggESPFolder.Name = "DiceHubEggESP"
    EggESPFolder.Parent = game:GetService("CoreGui")
    return EggESPFolder
end

local function clearEggESP()
    for uid, object in pairs(EggESPTags) do
        pcall(function() object:Destroy() end)
        EggESPTags[uid] = nil
    end
    if EggESPFolder then
        pcall(function() EggESPFolder:Destroy() end)
        EggESPFolder = nil
    end
end

local function getEggESPSnapshot()
    if not EggSnapshotESP or not EggSnapshotESP:IsA("RemoteFunction") then return {} end
    local ok, result = pcall(function() return EggSnapshotESP:InvokeServer() end)
    if not ok or type(result) ~= "table" or type(result.Records) ~= "table" then return {} end

    local eggs = {}
    for _, egg in pairs(result.Records) do
        if type(egg) == "table" and egg.State == "Slot" and egg.Uid then
            local part = getEggESPPart(egg.Uid)
            local position
            if part then
                position = part.Position
            elseif egg.BoundsCFrame then
                position = egg.BoundsCFrame.Position
            elseif egg.BottomCFrame then
                position = egg.BottomCFrame.Position
            end
            if position then
                local rarity = egg.Rarity or egg.RarityName or egg.Tier or egg.RarityId
                if type(rarity) == "table" then
                    rarity = rarity.DisplayName or rarity.Name or rarity.Id
                end
                eggs[#eggs+1] = {
                    uid = egg.Uid,
                    rarity = rarity,
                    label = egg.Label or "Egg",
                    mutated = egg.BaseMutation ~= nil
                        or (type(egg.Mutations) == "table" and next(egg.Mutations) ~= nil),
                }
            end
        end
    end
...
local function createEggESP(egg)
    local model = getEggESPModel(egg.uid)
    local part = getEggESPPart(egg.uid)
    if not model or not part then return nil end

    local color = EggESPRarityColors[egg.rarity] or Color3.fromRGB(255,255,255)
    local folder = Instance.new("Folder")
    folder.Name = tostring(egg.uid)

    local highlight = Instance.new("Highlight")
    highlight.Adornee = model
    highlight.FillColor = color
    highlight.OutlineColor = egg.mutated and Color3.fromRGB(255,150,60) or color
    ...
    label.Text = (egg.rarity or egg.label or "Egg") .. (egg.mutated and "  MUT" or "")
    label.Parent = billboard

    folder.Parent = getEggESPFolder()
    return folder
end
...
task.spawn(function()
    while h.alive do
        if EggESPEnabled then
            local data = getEggESPSnapshot()
            local active = {}
            for _, egg in ipairs(data) do
                active[egg.uid] = true
                local object = EggESPTags[egg.uid]
                if not object or not object.Parent or not getEggESPPart(egg.uid) then
                    if object then pcall(function() object:Destroy() end) end
                    EggESPTags[egg.uid] = createEggESP(egg)
                end
            end
            for uid, object in pairs(EggESPTags) do
        ...
        elseif next(EggESPTags) then
            clearEggESP()
        end
        task.wait(EggESPUpdate)
    end
    clearEggESP()
end)
...
local function oM(...)
    ...
    -- All controls are created directly on the tab. This avoids section/container
    -- compatibility issues and guarantees the elements are instantiated.
    local updating=false
    local teleportToggle
    pcall(function()
        teleportToggle=FarmTab:Toggle({
            Title="Auto Steal (Teleport)",
            Desc="Teleport to steal eggs continuously",
    ...
    pcall(function()
        FarmTab:Toggle({
            Title="Auto Steal",
            Desc="Smoothly move to eggs using the original Tween farm method",
            Value=h.pureTweenFarm==true,
            Callback=function(value)
                if updating then return end
                if value then
                    T4("TWEEN")
                elseif Y4=="TWEEN" or h.pureTweenFarm then
                    T4("NONE")
                end
            end,
        })
    ...
    pcall(function()
        FarmTab:Toggle({
            Title="Egg ESP",
            Desc="Show egg rarity, mutation and highlight through the map",
            Value=false,
            Callback=function(value)
                EggESPEnabled=value==true
                if not EggESPEnabled then clearEggESP() end
            end,
        })
    end)
    ...
    W4=function(value)
        pcall(function()
            ...
            if teleportToggle then
                if teleportToggle.Set then
                    teleportToggle:Set(value==true)
                elseif teleportToggle.SetValue then
    ...
    -- Extra visible button for testing the UI without changing the farm logic.
    pcall(function()
        FarmTab:Button({
    ...
    pcall(function()
        WindUI:Notify({Title="Dice Hub",Content="Auto Farm loaded",Duration=3})
    end)
...
H( "[+] Initializing Dice Hub x WindUI (Steal an Egg Edition)..." )oM()task.spawn (function(...) task.wait ( 0.5 )A4()b4( true )C4()
    if o.Character then
        z4(o.Character )
    end
