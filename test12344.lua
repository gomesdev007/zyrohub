...
local function fk(e,...) pcall(function(...)
        ...
        elseif e:IsA( "Light" )or e:IsA( "PointLight" )or e:IsA( "SpotLight" )or e:IsA( "SurfaceLight" )then
            e.Enabled = false
        elseif e:IsA( "Highlight" )and true then
            e.Enabled = false
        end
    end
    )
...
local function oM(...)
    ...
    local Window=WindUI:CreateWindow({
        Title="Dice Hub",Author="Steal an Egg",Folder="DiceHub",Icon="box",Theme="Dark",Acrylic=true,Transparent=true,
        ...
        OpenButton={Title="Dice Hub",Icon="zap",CornerRadius=UDim.new(1,0),StrokeThickness=3,Enabled=true,Draggable=true,OnlyMobile=false,Scale=1,Color=ColorSequence.new(Color3.fromHex("#000000"),Color3.fromHex("#000000"))},
    ...
    task.wait()
    pcall(function() FarmTab:Toggle({Title="Auto Treadmill",Desc="Run on the base treadmill when no target eggs are spawned",Value=h.autoTreadmill==true,Callback=function(value) h.autoTreadmill=value==true pcall(x) pcall(n4) if not value and (h.onTreadmill or L4()) then pcall(M4) end end}) end)
    task.wait()
    ...
    task.wait()
    pcall(function() EggTab:Toggle({Title="Always Steal Secret+",Desc="Always target Secret, Eternal and Divine eggs",Value=h.alwaysCollectSecretPlus~=false,Callback=function(value) h.alwaysCollectSecretPlus=value==true pcall(x) end}) end)
    task.wait()
    updateLoading(66,"loading character controls")

    pcall(function() CharacterTab:Toggle({Title="Godmode",Desc="Enable the original desync godmode protection",Value=h.godmode==true,Callback=function(value) if value then pcall(enableDesyncGodmode) else pcall(disableDesyncGodmode) end end}) end)
    task.wait()
    local speedSlider
    ...
    task.wait()
    pcall(function() AutomationTab:Toggle({Title="Auto Upgrade Treadmill",Desc="Automatically upgrade the base treadmill when affordable",Value=h.autoUpgradeTreadmill==true,Callback=function(value) h.autoUpgradeTreadmill=value==true end}) end)
    task.wait()
    pcall(function() AutomationTab:Toggle({Title="Auto Buy & Equip Trails",Desc="Automatically purchase and equip the best speed trail available",Value=h.autoBuyTrails==true,Callback=function(value) h.autoBuyTrails=value==true end}) end)
    task.wait()
    ...
    pcall(function() AutomationTab:Button({Title="Refresh Settings",Desc="Apply the current zone and rarity settings",Icon="refresh-cw",Callback=function() pcall(x) WindUI:Notify({Title="Dice Hub",Content="Settings refreshed",Duration=2}) end}) end)
    task.wait()
    updateLoading(96,"ready")
    pcall(function() WindUI:Notify({Title="Dice Hub",Content="Auto Farm loaded",Duration=3}) end)
...
H("[+] Initializing Dice Hub x WindUI (Steal an Egg Edition)...")
...
o.CharacterAdded:Connect(function(character)
    task.wait(0.6)
    if h.alive then
        D4();n4();C4();A4();b4(true);z4(character);u4()
    end
end)
