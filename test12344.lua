...
[ "autoFarmLoop" ]= false ,[ "pureTweenFarm" ]= false ,[ "manualStealReturn" ]= false ,[ "manualStealReturnConnection" ]=nil,[ "safeReturnButtonGui" ]=nil,[ "safeReturnButton" ]=nil,[ "safeReturnKeyConnection" ]=nil,[ "safeReturnKey" ]=Enum.KeyCode.RightControl,[ "safeReturnKeyName" ]="RightControl";
...
[ "teleporting" ]= false ,[ "isReturning" ]= false ,[ "manualSafeReturn" ]= false ;
...
Q4=function(speed,sessionId,forceManual,...)
    ...
    -- Dice Hub return movement: the Return state drives Humanoid:Move toward
    -- the safe target, while using the same treadmill/plot obstacle reroute.
    local safeTarget=s4()
    if typeof(safeTarget)~="Vector3" then return false end
    local laneZ=safeTarget.Z
    local moveSpeed=math.clamp(tonumber(speed) or tonumber(h.glideSpeed) or 350,50,1300)
    local previousAutoRotate=humanoid.AutoRotate
    humanoid.AutoRotate=true
    h.isReturning=true
    h.stateTime=os.clock()

    local function getTreadmillBounds()
        local minX,maxX,minZ,maxZ,sumY,count
        local function addPart(part)
            if not part or not part:IsA("BasePart") then return end
            local n=string.lower(part.Name)
            local pn=part.Parent and string.lower(part.Parent.Name) or ""
            if not n:find("treadmill",1,true) and not n:find("belt",1,true)
                and not pn:find("treadmill",1,true) and not pn:find("belt",1,true)
                and n~="treadmillbottom" then return end
            local cf,sz=part.CFrame,part.Size
            local hx=math.abs(cf.RightVector.X)*sz.X*.5+math.abs(cf.LookVector.X)*sz.Z*.5+math.abs(cf.UpVector.X)*sz.Y*.5
            local hz=math.abs(cf.RightVector.Z)*sz.X*.5+math.abs(cf.LookVector.Z)*sz.Z*.5+math.abs(cf.UpVector.Z)*sz.Y*.5
            local x,z=cf.Position.X,cf.Position.Z
            minX=minX and math.min(minX,x-hx) or x-hx
            maxX=maxX and math.max(maxX,x+hx) or x+hx
            minZ=minZ and math.min(minZ,z-hz) or z-hz
            maxZ=maxZ and math.max(maxZ,z+hz) or z+hz
            sumY=(sumY or 0)+cf.Position.Y
            count=(count or 0)+1
        end
        pcall(function()
            if h.tread and h.tread.Parent then addPart(h.tread) end
            if h.plot and h.plot.Parent then
                for _,d in ipairs(h.plot:GetDescendants()) do addPart(d) end
            end
            local renders=r:FindFirstChild("__ClientTreadmillRenders")
            if renders then
                for _,d in ipairs(renders:GetDescendants()) do
                    if d:IsA("BasePart") and (root.Position-d.Position).Magnitude<90 then addPart(d) end
                end
            end
        end)
        if not count or not minX then return nil end
        return {cx=(minX+maxX)*.5,cz=(minZ+maxZ)*.5,hx=(maxX-minX)*.5,hz=(maxZ-minZ)*.5,y=sumY/count}
    end

    local function inside(bounds,pos,pad)
        if not bounds or typeof(pos)~="Vector3" then return false end
        pad=pad or 0
        return math.abs(pos.X-bounds.cx)<=bounds.hx+pad and math.abs(pos.Z-bounds.cz)<=bounds.hz+pad
    end

    local function crosses(bounds,p1,p2,pad)
        if not bounds or typeof(p1)~="Vector3" or typeof(p2)~="Vector3" then return false end
        pad=pad or 0
        local x1,z1,x2,z2=p1.X,p1.Z,p2.X,p2.Z
        local lx,lz=bounds.cx-bounds.hx-pad,bounds.cz-bounds.hz-pad
        local rx,rz=bounds.cx+bounds.hx+pad,bounds.cz+bounds.hz+pad
        if (x1<lx and x2<lx) or (x1>rx and x2>rx) or (z1<lz and z2<lz) or (z1>rz and z2>rz) then return false end
        if inside(bounds,p1,pad) or inside(bounds,p2,pad) then return true end
        local dx,dz=x2-x1,z2-z1
        for n=1,8 do
            local f=n/9
            if inside(bounds,Vector3.new(x1+dx*f,0,z1+dz*f),pad) then return true end
        end
        return false
    end

    local function diceWaypoint(target,current)
        local bounds=getTreadmillBounds()
        if not bounds then return target end
        if (Vector3.new(safeTarget.X,0,safeTarget.Z)-Vector3.new(target.X,0,target.Z)).Magnitude<=62 then return target end
        if inside(bounds,target,12) or crosses(bounds,current,target,8) then
            local safe=Vector3.new(safeTarget.X,0,safeTarget.Z)
            local center=Vector3.new(bounds.cx,current.Y,bounds.cz)
            local dir=Vector3.new(safe.X-center.X,0,safe.Z-center.Z)
            if dir.Magnitude>=2 then
                dir=dir.Unit
                local wp=Vector3.new(bounds.cx+dir.X*32,current.Y,bounds.cz+dir.Z*32)
                if (current-wp).Magnitude>=12 then return wp end
            end
        end
        return target
    end
    ...
    local stillFor=0
    local waypoint=safeTarget

    while h.alive and h.isReturning and os.clock()<deadline do
        if sessionId and O4~=sessionId then
            if humanoid then humanoid:Move(Vector3.zero,false) end
            humanoid.AutoRotate=previousAutoRotate
            h.isReturning=false
            return false
        end
        if not forceManual and not h.manualSafeReturn and not h.pureTweenFarm and not h.autoFarmLoop then
            if humanoid then humanoid:Move(Vector3.zero,false) end
            humanoid.AutoRotate=previousAutoRotate
            h.isReturning=false
            return false
        ...
        if (Vector3.new(pos.X,0,laneZ)-Vector3.new(safeTarget.X,0,safeTarget.Z)).Magnitude<=4 or (safeTarget-pos).Magnitude<=6 then
            break
        end

        waypoint=diceWaypoint(safeTarget,pos)
        local delta=Vector3.new(waypoint.X-pos.X,0,waypoint.Z-pos.Z)
        ...
        if dist>1.2 then
            ...
            humanoid.Sit=false
            humanoid.AutoRotate=true
            humanoid.WalkSpeed=math.clamp(moveSpeed,16,1300)
            humanoid:Move(delta.Unit,false)
        else
            humanoid:Move(Vector3.zero,false)
            if waypoint~=safeTarget then waypoint=safeTarget end
        end

        local dt=y.Heartbeat:Wait()
        local nowPos=root.Position
        if (nowPos-lastPos).Magnitude<1.2 then
            stillFor=stillFor+dt
        else
            stillFor=0
        end
        lastPos=nowPos

        if stillFor>0.85 then
            pcall(function() humanoid.Jump=true humanoid.AutoJumpEnabled=true end)
        end
        if stillFor>2.6 then
            -- Same reset strategy used by Dice Hub: move forward from the
            -- current position and recover to floor/safe position if needed.
            local floorY=math.max(0,nowPos.Y-3)
            local targetPos=Vector3.new(nowPos.X+6,floorY,nowPos.Z)
            local dir=Vector3.new(waypoint.X-nowPos.X,0,waypoint.Z-nowPos.Z)
            if dir.Magnitude>8 then
                dir=dir.Unit
                targetPos=Vector3.new(nowPos.X+dir.X*16,floorY,nowPos.Z+dir.Z*16)
            end
            pcall(function()
                root.Anchored=false
        ...
        h.statusText=waypoint==safeTarget
            and string.format("Returning to Safe Area (%.0f studs)",dist)
            or "Returning to Safe Area [REROUTE]"
    end

    humanoid:Move(Vector3.zero,false)
    pcall(function()
        root.CFrame=CFrame.new(safeTarget.X,safeTarget.Y,safeTarget.Z)
        root.AssemblyLinearVelocity=Vector3.zero
        root.AssemblyAngularVelocity=Vector3.zero
    end)
    humanoid.AutoRotate=previousAutoRotate
...
local function returnToSafeAreaManual(source)
    ...
    -- Use the exact Auto Steal Safe Line routine. No target scan or egg selection is performed here.
    local oldPure=h.pureTweenFarm
    local oldWarp=h.autoFarmLoop
    local oldManual=h.manualSafeReturn
    h.manualSafeReturn=true
    h.pureTweenFarm=true
    h.autoFarmLoop=false
    h.statusText="[SafeReturn] Returning to Safe Area..."
    H("[SafeReturn] "..tostring(source or "button").." -> exact Auto Steal return movement.")

    local ok,result=pcall(function()
        return Q4(h.glideSpeed,O4)
    end)

    h.manualSafeReturn=oldManual
    h.pureTweenFarm=oldPure
    h.autoFarmLoop=oldWarp

    if not ok then
        h.isReturning=false
        warn("[SafeReturn] Q4 failed: "..tostring(result))
        return false
    end
    return result==true
end

local function setSafeReturnKey(keyCode)
    if h.safeReturnKeyConnection then
        pcall(function() h.safeReturnKeyConnection:Disconnect() end)
        h.safeReturnKeyConnection=nil
    end
    if typeof(keyCode)=="EnumItem" and keyCode.EnumType==Enum.KeyCode then
        h.safeReturnKey=keyCode
        h.safeReturnKeyName=keyCode.Name
    end
    if not h.alive then return end
    local ok,conn=pcall(function()
        return w.InputBegan:Connect(function(input,gameProcessed)
            if gameProcessed or not h.alive then return end
            if input.UserInputType==Enum.UserInputType.Keyboard and input.KeyCode==h.safeReturnKey then
                task.spawn(function() returnToSafeAreaManual("key: "..tostring(h.safeReturnKeyName)) end)
            end
        end)
    end)
    if ok and conn then h.safeReturnKeyConnection=conn end
end
...
local function createSafeReturnButton()
    ...
    setSafeReturnKey(h.safeReturnKey or Enum.KeyCode.RightControl)
    return true
end
...
local function oM(...)
    ...
    local Window=WindUI:CreateWindow({
        ...
        ToggleKey=Enum.KeyCode.RightShift,Resizable=true,AutoScale=true,NewElements=true,BackgroundImageTransparency=0.65,HideSearchBar=false,
    ...
    task.wait()
    local safeReturnKeyNames={"RightControl","RightShift","LeftControl","LeftShift","F","G","H","J","K","L","Z","X","C","V","B","N","M"}
    pcall(function() SettingsTab:Dropdown({Title="Safe Return Key",Desc="Choose the keyboard key that activates the floating Safe Area button",Values=safeReturnKeyNames,Value=h.safeReturnKeyName or "RightControl",AllowNone=false,SearchBarEnabled=true,Callback=function(value)
        local name=type(value)=="table" and value[1] or value
        if type(name)=="string" and Enum.KeyCode[name] then
            setSafeReturnKey(Enum.KeyCode[name])
        end
    end}) end)
    task.wait()
