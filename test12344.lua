...
Q4=function(e,r,...)
    ...
    local s=os.clock ()+ 15
    while h.alive and(h.isReturning and os.clock ()<s)do
        local manualSafeReturn = (r == "MANUAL_SAFE")
        if r and not manualSafeReturn and O4~=r then
        ...
        if not manualSafeReturn and not h.pureTweenFarm and not h.autoFarmLoop and not h.manualStealReturn then
...
local function returnToSafeAreaManual(source)
    ...
    if h.isReturning then return false end
    ...
    local ok,result=pcall(function()
        return Q4(h.glideSpeed,"MANUAL_SAFE")
    end)
    if not ok then
        h.isReturning=false
