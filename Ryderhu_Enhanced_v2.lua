...
--// IDs DA BOLINHA
local BALL_IMAGE_1 = "rbxassetid://138175358452197" -- Ryderhub
local BALL_IMAGE_1_THUMB = "rbxthumb://type=Asset&id=138175358452197&w=420&h=420"
local BALL_IMAGE_2 = "rbxassetid://71571085068909" -- fallback
...
OpenButton.Position = UDim2.new(0,14,0.5,-29)
OpenButton.BackgroundColor3 = Color3.fromRGB(32,32,38)
...
-- Pequeno brilho sobre a imagem.
local BallGlow = Instance.new("UIGradient")
BallGlow.Rotation = 90
BallGlow.Transparency = NumberSequence.new({
    NumberSequenceKeypoint.new(0, 0.88),
    NumberSequenceKeypoint.new(0.5, 0.96),
    NumberSequenceKeypoint.new(1, 0.88)
})
BallGlow.Parent = OpenButton

-- Carregamento da bolinha: thumbnail do asset + fallback direto.
-- O fundo continua totalmente opaco caso nenhuma imagem carregue.
task.spawn(function()
    local candidates = {
        "rbxthumb://type=Asset&id=138175358452197&w=420&h=420",
        BALL_IMAGE_2,
        BALL_IMAGE_1
    }

    for _, imageId in ipairs(candidates) do
        OpenButton.Image = imageId
        local ok = pcall(function()
            ContentProvider:PreloadAsync({OpenButton})
        end)
        if ok then
            break
        end
...
Main.Size =
    UDim2.fromOffset(278,340)

Main.Position =
    UDim2.new(0.5,-139,0.5,-170)
...
MainGradient.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(1, Color3.fromRGB(24,13,16))
})
MainGradient.Parent = Main

local Accent = Instance.new("Frame")
Accent.Size = UDim2.new(1,-28,0,2)
Accent.Position = UDim2.fromOffset(14,56)
Accent.BackgroundColor3 = Color3.fromRGB(225,55,65)
...
--//====================================================
--// DRAG DA BOLINHA - PC + MOBILE
--//====================================================

local BallDragging = false
local BallDragStart
local BallStartPosition
local BallDragInput
...
local function UpdateBallDrag(Input)
    ...
    OpenButton.Position = UDim2.new(
        BallStartPosition.X.Scale,
        BallStartPosition.X.Offset + Delta.X,
        BallStartPosition.Y.Scale,
        BallStartPosition.Y.Offset + Delta.Y
    )
end
...
OpenButton.InputBegan:Connect(function(Input)
    if Input.UserInputType == Enum.UserInputType.MouseButton1
        ...
        BallStartPosition = OpenButton.Position
        BallDragInput = Input

        TweenService:Create(BallScale, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {
            Scale = 0.92
        }):Play()
        ...
        Input.Changed:Connect(function()
            if Input.UserInputState == Enum.UserInputState.End then
                ...
                TweenService:Create(BallScale, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                    Scale = 1
                }):Play()
            end
        end)
...
UserInputService.InputChanged:Connect(function(Input)
    if BallDragging and Input == BallDragInput then
...
local function CreateSlider(
    ...
    Container.BackgroundColor3 = Color3.fromRGB(19,19,23)
...
local function CreateButton(Y, Text)
    ...
    ButtonStroke.Transparency = 0.42
    ButtonStroke.Parent = Button

    local ButtonGradient = Instance.new("UIGradient")
    ButtonGradient.Rotation = 90
    ButtonGradient.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(48,48,57)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(29,29,36))
    })
...
Status.Position =
    UDim2.fromOffset(14,306)

Status.BackgroundColor3 = Color3.fromRGB(19,19,23)
