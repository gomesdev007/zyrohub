...
--==================================================
-- LIGHTWEIGHT LOADING + WINDUI INTERFACE
-- Egg ESP removed completely.
--==================================================
local LoadingGui=nil
local LoadingFill=nil
local LoadingPercent=nil
local LoadingTitle=nil
local LoadingAlive=true
local StartupReady=false
local StartupStage=0

local function createLoadingScreen()
    pcall(function()
        local parent=o:FindFirstChildOfClass("PlayerGui") or game:GetService("CoreGui")
        LoadingGui=Instance.new("ScreenGui")
        LoadingGui.Name="DiceHubLoading"
        ...
        card.Size=UDim2.fromOffset(330,118)
        ...
        LoadingTitle.Size=UDim2.new(1,-32,0,24)
        LoadingTitle.Position=UDim2.fromOffset(16,13)
        LoadingTitle.BackgroundTransparency=1
        LoadingTitle.Text="gomes hub"
        ...
        bar.Position=UDim2.fromOffset(16,70)
        bar.BackgroundColor3=Color3.fromRGB(38,38,45)
        ...
        LoadingPercent.Size=UDim2.new(1,-32,0,20)
        LoadingPercent.Position=UDim2.fromOffset(16,84)
        LoadingPercent.BackgroundTransparency=1
        LoadingPercent.Text="0%"
        LoadingPercent.TextColor3=Color3.fromRGB(190,190,200)
...
local function updateLoading(percent,status)
    percent=math.clamp(tonumber(percent) or 0,0,100)
    pcall(function()
        if LoadingFill then
            LoadingFill.Size=UDim2.new(percent/100,0,1,0)
        end
        if LoadingPercent then
            LoadingPercent.Text=string.format("%d%%",math.floor(percent+0.5))
        end
        if LoadingGui then
            local label=LoadingGui:FindFirstChild("Status",true)
            if label then label.Text=status or "loading" end
        end
    end)
end
    ...
    pcall(function()
        ...
        for _,obj in ipairs(LoadingGui:GetDescendants()) do
            if obj:IsA("Frame") then
                TweenService:Create(obj,TweenInfo.new(0.55,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{BackgroundTransparency=1}):Play()
            elseif obj:IsA("TextLabel") then
                TweenService:Create(obj,TweenInfo.new(0.55,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{TextTransparency=1}):Play()
            elseif obj:IsA("UIStroke") then
                TweenService:Create(obj,TweenInfo.new(0.55),{Transparency=1}):Play()
            end
        end
        task.wait(0.6)
        LoadingGui:Destroy()
...
local function T(...)
    -- Only non-activation preferences are restored between executions.
    -- Feature toggles intentionally start disabled every time the script runs.
    local e=nil
    pcall(function(...)
        if isfile and readfile and a and isfile(z) then
            local raw=readfile(z)
            if raw and raw~="" then
                local decoded=a:JSONDecode(raw)
                if type(decoded)=="table" then e=decoded end
            end
        end
    end)
    ...
    if type(e.selectedRarities)~="table" then e.selectedRarities=y else
        for _,w in ipairs(X) do if e.selectedRarities[w]==nil then e.selectedRarities[w]=(y[w]==true) end end
    end
...
local function x(...)
    pcall(function(...)
        if writefile and a and h then
            -- Deliberately save only selection/preferences, never feature activation states.
            local r={
                ["selectedZones"]=h.selectedZones or {},
                ["selectedRarities"]=h.selectedRarities or {},
                ["minRarityTier"]=h.minRarityTier or 2
            }
            writefile(z,a:JSONEncode(r))
        end
    end)
end
local W=T()h={[ "godmode" ]= false ,[ "autoGlide" ]= false ,[ "autoHatch" ]= false ;
[ "autoPlaceEvery5" ]= false ;
[ "batchStealCount" ]= 0 ,[ "stealCount" ]= 0 ,[ "isBatchPlacing" ]= false ,[ "isHatching" ]= false ;
[ "autoFarmLoop" ]= false ,[ "pureTweenFarm" ]= false ,[ "manualStealReturn" ]= false ,[ "manualStealReturnConnection" ]=nil,[ "automaticSteal2" ]=false,[ "safeReturnButtonGui" ]=nil,[ "safeReturnButton" ]=nil,[ "keybinds" ]={},[ "keybindCapture" ]=nil,[ "keybindInputConnection" ]=nil;
...
[ "performanceMode" ]=false,[ "disable3D" ]=false,[ "antiAFK" ]=false,[ "onTreadmill" ]= false ,[ "lastTreadmillMount" ]= 0 ,[ "laneZ" ]= -360 ,[ "swapped" ]= false ;
...
[ "gui" ]=nil;
[ "batAura" ]=false,[ "antiTrap" ]=false,[ "antiRagdoll" ]=false,[ "instantPickup" ]=false,[ "instantPickupConnection" ]=nil,[ "batAuraConnection" ]=nil,[ "antiTrapConnection" ]=nil,[ "antiRagdollConnections" ]={};
[ "alive" ]= true ,[ "plot" ]=nil;
[ "pen" ]=nil,[ "origin" ]=nil;
[ "tread" ]=nil}
local m
...
if typeof(hookmetamethod)== "function" and not _G._DesyncAntiRagdollHooked then
    ...
    local e e=hookmetamethod(game, "__newindex" ,safeNewCClosure(function(r,y,u,...)
        if not executorCheckCaller()and typeof(r)== "Instance" then
            if h.antiRagdoll and r:IsA( "Motor6D" )and(y== "Enabled" and u== false )then
                return nil
            end
            if h.antiRagdoll and r:IsA( "Humanoid" )then
...
z4=function(e,...)
    ...
    for e,y in ipairs(e:GetDescendants())do
        if y:IsA( "Motor6D" )then
            local conn=(y:GetPropertyChangedSignal( "Enabled" )):Connect(function(...)
                if h.antiRagdoll and not y.Enabled then y.Enabled = true end
            end)
            table.insert(h.antiRagdollConnections,conn)
        end
    end
    table.insert(h.antiRagdollConnections,e.DescendantAdded:Connect(function(y,...)
        if not h.antiRagdoll then return end
    ...
    table.insert(h.antiRagdollConnections,e.ChildAdded:Connect(function(e,...)
...
A4=function(...)
    ...
    if h.antiRagdoll then z4(e) end
    return true
end
t4=function(...)
...
Q4=function(speed,sessionId,forceManual,...)
    local character=o.Character
    local root=character and character:FindFirstChild("HumanoidRootPart")
    local humanoid=character and character:FindFirstChildOfClass("Humanoid")
    if not root then return false end

    local fixedTarget=Vector3.new(590,71,-368)
    local moveSpeed=math.clamp(tonumber(speed) or tonumber(h.glideSpeed) or 350,100,1300)
    h.isReturning=true
    h.stateTime=os.clock()
    h.statusText="[SafeReturn] Moving to fixed position..."
    ...
    while h.alive and h.isReturning and os.clock()<deadline do
        if sessionId and O4~=sessionId and not forceManual then
            h.isReturning=false
        ...
        local delta=fixedTarget-current
        local distance=delta.Magnitude
        if distance<=3 then break end

        local dt=y.Heartbeat:Wait()
        if not h.alive or not h.isReturning then break end
        current=root.Position
        delta=fixedTarget-current
        distance=delta.Magnitude
        if distance<=3 then break end

        local step=math.min(distance,moveSpeed*math.clamp(dt,0.01,0.1))
        local nextPosition=current+delta.Unit*step

        -- Keep the movement horizontal when possible; do not force jumping.
        local obstacles={}
        pcall(function() obstacles=i4() or {} end)
        for _,part in ipairs(obstacles) do
            if part and part:IsA("BasePart") then
                local horizontal=(Vector3.new(nextPosition.X,0,nextPosition.Z)-Vector3.new(part.Position.X,0,part.Position.Z)).Magnitude
                if horizontal<14 and math.abs(nextPosition.Y-part.Position.Y)<10 then
                    local side=Vector3.new(-(nextPosition.Z-part.Position.Z),0,nextPosition.X-part.Position.X)
                    if side.Magnitude>0.05 then
                        nextPosition=nextPosition+side.Unit*12
                    end
                    break
                end
            end
        ...
        local lookDelta=Vector3.new(fixedTarget.X-nextPosition.X,0,fixedTarget.Z-nextPosition.Z)
        local lookVector=lookDelta.Magnitude>0.05 and lookDelta.Unit or Vector3.new(0,0,-1)
        root.CFrame=CFrame.lookAt(nextPosition,nextPosition+lookVector)
        root.AssemblyLinearVelocity=Vector3.zero
        root.AssemblyAngularVelocity=Vector3.zero
        pcall(function() humanoid:Move(Vector3.zero,true) end)
        h.statusText=string.format("[SafeReturn] %.0f studs remaining",distance)
    end

    -- Final stop: no rotating, no drifting, no extra movement.
    if h.alive and h.isReturning then
        local finalCFrame=CFrame.new(fixedTarget.X,fixedTarget.Y,fixedTarget.Z)
    ...
    h.delivering=false
    h.onTreadmill=false
    h.statusText="Fixed return position reached (590, 71, -368). Movement stopped."
    return true
end
...
local Ck=os.clock ()task.spawn (function(...)
    while h.alive do
        if not StartupReady then task.wait(0.1) continue end
        local r,y=pcall(function(...)
...
local function jM(...)
    ...
    local w=Instance.new ( "TextLabel" )w.Size =UDim2.new ( 1 , -28 , 0 , 24 )w.Position =UDim2.new ( 0 , 14 , 0 , 14 )w.BackgroundTransparency = 1 w.Text = "gomes hub" w.TextColor3 =Color3.fromRGB ( 245 , 248 , 255 )w.TextSize = 18 w.Font =Enum.Font.GothamBold w.TextXAlignment =Enum.TextXAlignment.Left w.AutoLocalize = false w.Parent =r
    ...
    local t=Instance.new ( "TextLabel" )t.Size =UDim2.new ( 1 , -28 , 0 , 16 )t.Position =UDim2.new ( 0 , 14 , 0 , 94 )t.BackgroundTransparency = 1 t.Text = "steal an egg" t.TextColor3 =Color3.fromRGB ( 130 , 140 , 165 )t.TextSize = 11 t.Font =Enum.Font.Gotham t.TextXAlignment =Enum.TextXAlignment.Left t.AutoLocalize = false t.Parent =r task.spawn (function(...)
        for y= 1 , 100 , 1 do
            if not e.Parent then
                break
            end
...
-- Auto Equip Best, based directly on the supplied standalone file.
local AutoEquipBest=false
local AutoEquipBestRunning=true
local AutoEquipNetworking=j:FindFirstChild("Packages") and j.Packages:FindFirstChild("Networking")
local WearBestPath="RF/Haul/WearBest"
local function getAutoEquipRemote()
    if not AutoEquipNetworking then return nil end
    return AutoEquipNetworking:FindFirstChild(WearBestPath)
end
local function autoEquipCharacterReady()
...
local function wearBest()
    local remote=getAutoEquipRemote()
...
_G.AutoEquipBest={
    Enable=function() setAutoEquipBest(true) end,
    Disable=function() setAutoEquipBest(false) end,
    Toggle=function() setAutoEquipBest(not AutoEquipBest) end,
    EquipBestNow=equipBestNow,
    IsEnabled=function() return AutoEquipBest end,
    Stop=function() AutoEquipBest=false AutoEquipBestRunning=false end,
}

task.spawn(function()
    while AutoEquipBestRunning and h.alive do
        if not StartupReady then task.wait(0.1) continue end
        if AutoEquipBest and autoEquipCharacterReady() then
            pcall(wearBest)
        end
        task.wait(6)
    end
...
updateLoading(12,"loading interface")

aM=function(...) h.alive=false pcall(setInstantPickup,false) pcall(setManualStealReturn,false) pcall(function() if h.keybindInputConnection then h.keybindInputConnection:Disconnect() h.keybindInputConnection=nil end end) pcall(function() if h.safeReturnButtonGui then h.safeReturnButtonGui:Destroy() h.safeReturnButtonGui=nil end end) pcall(setBatAura,false) pcall(setAntiTrap,false) pcall(setAntiRagdoll,false) pcall(function() if h.stealCounterGui then h.stealCounterGui:Destroy() end end) pcall(Ik)pcall(Ak)pcall(function() y:Set3dRenderingEnabled(true) end)pcall(D4)pcall(u4)
    ...
    pcall(function()
        for _,obj in ipairs(game.CoreGui:GetChildren()) do
            if obj.Name:find("Dice_") or obj.Name:find("DesyncSniperUI") or obj.Name:find("WindUI") or obj.Name:find("DiceHubLoading") then obj:Destroy() end
        end
    end)
end

local function setInstantPickup(enabled)
    h.instantPickup=enabled==true
    if h.instantPickupConnection then
        pcall(function() h.instantPickupConnection:Disconnect() end)
        h.instantPickupConnection=nil
    end
    if not h.instantPickup then return true end
    local localPlayer=o
    ...
    local ok,conn=pcall(function()
        return k.PromptButtonHoldBegan:Connect(function(ProximityPrompt,Player)
            if Player==localPlayer and tostring(ProximityPrompt)=="CarryAreaEgg" then
                ProximityPrompt.HoldDuration=0
            end
        end)
    end)
    if ok and conn then
        h.instantPickupConnection=conn
        print("Instant Pickup init success")
        return true
    end
    h.instantPickup=false
    warn("failed to create connection")
    return false
end


local function returnToSafeAreaManual(source)
    if not h.alive then return false end
    h.glidingToTarget=false
    h.securingEgg=false
    h.delivering=false
    h.teleporting=false
    ...
    h.statusText="[SafeReturn] Going to fixed return position..."
    H("[SafeReturn] "..tostring(source or "button").." -> fixed position (590, 71, -368).")

    local ok,result=pcall(function()
        return Q4(h.glideSpeed,O4,true)
    end)
    if not ok or result~=true then
        h.isReturning=false
        if not ok then warn("[SafeReturn] Q4 failed: "..tostring(result)) end
        return false
    end

    -- Automatic Steal 2 belongs to Auto Steal Manual only.
    -- After reaching the fixed position, wait 0.5s, then use the normal
    -- safe-area return movement. If it is disabled, stop at the fixed point.
    if source=="Auto Steal Manual" and h.automaticSteal2 and h.manualStealReturn and h.alive then
        h.statusText="[Automatic Steal 2] Fixed position reached. Waiting 0.25s..."
        task.wait(0.25)
        if not h.alive or not h.manualStealReturn or not h.automaticSteal2 then
            return true
        end
        ...
        -- Automatic Steal 2: after the fixed stop, use ONLY Humanoid:MoveTo
        -- for the trip from the fixed point to the actual Safe Area.
        local safeTarget=s4()
        if typeof(safeTarget)~="Vector3" then
            return false
        end

        h.isReturning=true
        h.statusText="[Automatic Steal 2] Moving to Safe Area with MoveTo..."
        H("[Automatic Steal 2] 0.25s complete -> MoveTo Safe Area.")
        ...
        local deadline=os.clock()+30
        local reached=false
        while h.alive and h.manualStealReturn and h.automaticSteal2 and os.clock()<deadline do
            humanoid:MoveTo(safeTarget)
            local finished=humanoid.MoveToFinished:Wait()
            if not h.alive or not h.manualStealReturn or not h.automaticSteal2 then
                break
            end
            if (root.Position-safeTarget).Magnitude<=7 then
                reached=true
                break
            end
        ...
        -- Stop completely at the Safe Area; do not CFrame/teleport the final leg.
        ...
        h.delivering=false
        h.onTreadmill=false
        h.statusText=reached and "Safe Area reached (MoveTo)." or "Safe Area movement stopped."
        return reached
    end
...
local function setupKeybindSystem(KeybindsTab, Window)
    local actions={
        {"Instant Pickup",function() setInstantPickup(not h.instantPickup) end},
        {"Auto Steal Manual",function() setManualStealReturn(not h.manualStealReturn) end},
        {"Automatic Steal 2",function() h.automaticSteal2=not h.automaticSteal2 end},
        {"Auto Steal (Teleport)",function() if h.autoFarmLoop then T4("NONE") else T4("WARP") end end},
        {"Auto Steal",function() if h.pureTweenFarm then T4("NONE") else T4("TWEEN") end end},
        {"Auto Place (Every 5)",function() h.autoPlaceEvery5=not h.autoPlaceEvery5 if not h.autoPlaceEvery5 then h.batchStealCount=0 end end},
        {"Auto Return",function() h.autoGlide=not h.autoGlide end},
        {"Auto Hatch",function() h.autoHatch=not h.autoHatch end},
        {"Auto Treadmill",function() h.autoTreadmill=not h.autoTreadmill if h.autoTreadmill then pcall(n4) elseif h.onTreadmill or L4() then pcall(M4) end end},
        {"Godmode",function() h.godmode=not h.godmode if h.godmode then pcall(enableDesyncGodmode) else pcall(disableDesyncGodmode) end end},
        {"Bat Aura",function() setBatAura(not h.batAura) end},
        {"Anti Trap",function() setAntiTrap(not h.antiTrap) end},
        {"Anti Ragdoll",function() setAntiRagdoll(not h.antiRagdoll) end},
        {"Auto Equip Best",function() setAutoEquipBest(not AutoEquipBest) end},
        {"Auto Upgrade Treadmill",function() h.autoUpgradeTreadmill=not h.autoUpgradeTreadmill end},
        {"Auto Buy & Equip Trails",function() h.autoBuyTrails=not h.autoBuyTrails end},
        {"Hide Not Enough Money",function() h.hideNotEnoughMoney=not h.hideNotEnoughMoney end},
        {"Safe Return",function() task.spawn(function() returnToSafeAreaManual("keybind") end) end},
        {"Equip Best Now",function() pcall(equipBestNow) end},
        {"Get Unstuck",function() pcall(M4) pcall(C4) pcall(D4) end},
        {"Server Hop",function() pcall(serverHop) end},
        {"Minimize Hub",function() pcall(function() if h.gui and h.gui.Toggle then h.gui:Toggle() end end) end},
        {"Test Safe Return",function() task.spawn(function() returnToSafeAreaManual("keybind test") end) end},
    }
    for _,entry in ipairs(actions) do
        local name,callback=entry[1],entry[2]
        h.keybinds[name]={callback=callback,key=nil,keyName="unbound",button=nil}
    end
    local captureLabel
    pcall(function() KeybindsTab:Section({Title="Keybinds"}) end)
    pcall(function() KeybindsTab:Paragraph({Title="How to bind",Content="Click a function, then press any keyboard key. It is saved instantly. Press the same key again to run the function."}) end)
    pcall(function() KeybindsTab:Paragraph({Title="Current selection",Content="No function selected. Click a function below to choose its key."}) end)
    for _,entry in ipairs(actions) do
        local name=entry[1]
        pcall(function()
            local btn=KeybindsTab:Button({Title=name.."  [unbound]",Desc="key: unbound — click here, then press any keyboard key",Icon="keyboard",Callback=function()
                h.keybindCapture=name
                pcall(function() WindUI:Notify({Title="keybind",Content="Press any keyboard key for: "..name,Duration=3}) end)
            end})
            h.keybinds[name].button=btn
        end)
        task.wait()
    end
    if h.keybindInputConnection then pcall(function() h.keybindInputConnection:Disconnect() end) end
    h.keybindInputConnection=w.InputBegan:Connect(function(input,gameProcessed)
        if not h.alive then return end
        if input.UserInputType~=Enum.UserInputType.Keyboard then return end
        local selected=h.keybindCapture
        if selected then
            if input.KeyCode==Enum.KeyCode.Unknown then return end
            local action=h.keybinds[selected]
            if action then
                action.key=input.KeyCode
                action.keyName=input.KeyCode.Name
                h.keybindCapture=nil
                pcall(function()
                    if action.button.SetTitle then action.button:SetTitle(selected.."  ["..input.KeyCode.Name.."]") end
                    if action.button.SetDesc then action.button:SetDesc("key: "..input.KeyCode.Name.." — click here to change it") end
                end)
                pcall(function() WindUI:Notify({Title="keybind saved",Content=selected.." = "..input.KeyCode.Name,Duration=2}) end)
            end
            return
        end
        for name,action in pairs(h.keybinds) do
            if action.key and input.KeyCode==action.key then
                task.spawn(function() pcall(action.callback) end)
            end
        end
    end)
end

local function createSafeReturnButton()
    if h.safeReturnButtonGui then pcall(function() h.safeReturnButtonGui:Destroy() end) end
    local parent=o:FindFirstChildOfClass("PlayerGui") or game:GetService("CoreGui")
    if not parent then return false end
    local sg=Instance.new("ScreenGui")
    sg.Name="GomesHubSafeReturn"
    ...
    button.Activated:Connect(function()
        ...
        busy=true
        task.spawn(function()
            pcall(function() returnToSafeAreaManual("button") end)
            task.wait(0.25)
            busy=false
        end)
    end)

    h.safeReturnButtonGui=sg
    h.safeReturnButton=button
    return true
end

local function setManualStealReturn(enabled)
    h.manualStealReturn=enabled==true
    if h.manualStealReturnConnection then
        pcall(function() h.manualStealReturnConnection:Disconnect() end)
        h.manualStealReturnConnection=nil
    end
    if not h.manualStealReturn then
        return true
    end
    local localPlayer=o
    if not localPlayer then return false end
    local ok,conn=pcall(function()
        return k.PromptTriggered:Connect(function(ProximityPrompt,Player)
            if not h.manualStealReturn or not h.alive or Player~=localPlayer then return end
            if h.isReturning or h.glidingToTarget or h.teleporting or h.delivering then return end
            task.spawn(function()
                if not h.manualStealReturn or not h.alive then return end
                h.statusText="[ManualSteal] Prompt completed! Returning to Safe Area..."
                H("[ManualSteal] Prompt completed. Using Auto Steal movement to Safe Area.")
                returnToSafeAreaManual("Auto Steal Manual")
            end)
        end)
    end)
    if ok and conn then
        h.manualStealReturnConnection=conn
        return true
    end
    h.manualStealReturn=false
    return false
end
local function setAntiRagdoll(enabled)
    h.antiRagdoll=enabled==true
    if h.antiRagdollConnections then
        for _,conn in ipairs(h.antiRagdollConnections) do pcall(function() conn:Disconnect() end) end
        h.antiRagdollConnections={}
    end
    if not h.antiRagdoll then return end
    local character=o.Character
    if character then pcall(z4,character) end
end
local function registerSteal(source)
    h.stealCount=(tonumber(h.stealCount) or 0)+1
    h.lastStealSource=tostring(source or "AutoSteal")
    pcall(function()
        if h.stealCounterLabel and h.stealCounterLabel.Parent then
            h.stealCounterLabel.Text="steals: "..tostring(h.stealCount)
        end
    end)
end
local BatAuraRunning=false
local AntiTrapRunning=false
local ServerHopBusy=false
local function getBatTool(character)
    if not character then return nil end
    for _,obj in ipairs(character:GetChildren()) do
        if obj:IsA("Tool") and (obj:GetAttribute("IsBat")==true or obj:GetAttribute("Bat")==true) then return obj end
    end
end
local function getCharacterRoot(player)
    ...
    if root and hum and hum.Health>0 then return root,hum,char end
end
local function findBatRemote()
    local remotes=j:FindFirstChild("Remotes")
    local bat=remotes and remotes:FindFirstChild("BatSwing")
    local trigger=bat and bat:FindFirstChild("Trigger")
    if trigger and trigger:IsA("RemoteEvent") then return trigger end
end
local function equipBat()
    local bat=getBatTool(o.Character)
    if bat then return bat end
    local backpack=o:FindFirstChildOfClass("Backpack")
    bat=getBatTool(backpack)
    local hum=o.Character and o.Character:FindFirstChildOfClass("Humanoid")
    if bat and hum then pcall(function() hum:EquipTool(bat) end) task.wait() return getBatTool(o.Character) or bat end
end
local function setBatAura(enabled)
    BatAuraRunning=enabled==true h.batAura=BatAuraRunning
    if h.batAuraConnection then pcall(function() h.batAuraConnection:Disconnect() end) h.batAuraConnection=nil end
    if not BatAuraRunning then return end
    local lastSwing=0 local range=17
    pcall(function()
        local cfg=j:FindFirstChild("Modules") cfg=cfg and cfg:FindFirstChild("BatController") cfg=cfg and cfg:FindFirstChild("Config")
        if cfg and cfg:IsA("ModuleScript") then local data=require(cfg) if type(data)=="table" then range=(tonumber(data.Range) or 15)+(tonumber(data.HitTolerance) or 2) if type(data.GetHitboxScalar)=="function" then local ok,val=pcall(data.GetHitboxScalar) if ok and type(val)=="number" and val>0 then range*=val end end end end
    end)
    h.batAuraConnection=y.Heartbeat:Connect(function()
        if not BatAuraRunning or not h.alive then return end
        local now=os.clock() if now-lastSwing<0.7 then return end
        local myRoot=getCharacterRoot(o) if not myRoot then return end
        local target=nil local best=range
        for _,plr in ipairs(e:GetPlayers()) do
            if plr~=o then local root=getCharacterRoot(plr) if root then local dist=(myRoot.Position-root.Position).Magnitude if dist<=best then target=plr best=dist end end end
        end
        if not target or not equipBat() then return end
        local remote=findBatRemote() if not remote then return end
        lastSwing=now
        pcall(function() remote:FireServer(target,string.format("%d:%d:%d",o.UserId,math.floor(now*1000),math.floor(workspace:GetServerTimeNow()*1000))) end)
    end)
end
local function setAntiTrap(enabled)
    AntiTrapRunning=enabled==true h.antiTrap=AntiTrapRunning
    if h.antiTrapConnection then pcall(function() h.antiTrapConnection:Disconnect() end) h.antiTrapConnection=nil end
    if not AntiTrapRunning then return end
    h.antiTrapConnection=y.Heartbeat:Connect(function()
        if not AntiTrapRunning or not h.alive then return end
        local root,hum,char=getCharacterRoot(o) if not root then return end
        if char:GetAttribute("IsTrapped")==true or root.Anchored or hum.JumpHeight<0.5 then
            pcall(function()
                char:SetAttribute("IsTrapped",nil) root.Anchored=false hum.PlatformStand=false hum.Sit=false hum.AutoRotate=true
                if hum.JumpHeight<0.5 then hum.JumpHeight=7.2 end
                local state=hum:GetState()
                if state==Enum.HumanoidStateType.Physics or state==Enum.HumanoidStateType.PlatformStanding or state==Enum.HumanoidStateType.Seated then hum:ChangeState(Enum.HumanoidStateType.Running) end
                local billboard=char:FindFirstChild("TrapBillboard",true) if billboard then billboard:Destroy() end
            end)
        end
    end)
end
local function getRequestFunction()
    ...
    if http_request and typeof(http_request)=="function" then return http_request end
end
local function serverHop()
    if ServerHopBusy or not h.alive then return false,"busy" end
    ServerHopBusy=true local ok=false local reason="request unavailable"
    pcall(function()
        local req=getRequestFunction() if not req then return end
        ...
        local body=response and (response.Body or response.body) if type(body)~="string" or body=="" then reason="empty server list" return end
        local data=a:JSONDecode(body) local choices={}
        if type(data)=="table" and type(data.data)=="table" then for _,srv in ipairs(data.data) do if type(srv)=="table" and srv.id and srv.id~=game.JobId and (tonumber(srv.playing) or 0)<(tonumber(srv.maxPlayers) or math.huge) then choices[#choices+1]=srv.id end end end
        if #choices==0 then reason="no other server found" return end
        local target=choices[math.random(1,#choices)] ok=true reason=target
        TeleportService:TeleportToPlaceInstance(game.PlaceId,target,o)
    end)
    ServerHopBusy=false return ok,reason
end
local function oM(...)
    if h.gui then pcall(function() h.gui:Destroy() end) h.gui=nil end

    updateLoading(16,"loading windui")
    task.wait()
    local WindUI
    local ok,result=pcall(function()
    ...
    if not WindUI then H("[!] WindUI could not be loaded") return end

    pcall(function()
        WindUI:AddTheme({Name="Dark",Accent=Color3.fromHex("#18181b"),Background=Color3.fromHex("#101010"),Outline=Color3.fromHex("#FFFFFF"),Text=Color3.fromHex("#FFFFFF"),Placeholder=Color3.fromHex("#7a7a7a"),Button=Color3.fromHex("#27272a"),Icon=Color3.fromHex("#a1a1aa")})
    end)
    updateLoading(24,"building window")
    task.wait()

    local camera=workspace.CurrentCamera
    local viewport=camera and camera.ViewportSize or Vector2.new(1280,720)
    local mobile=w.TouchEnabled and not w.KeyboardEnabled
    local width=mobile and math.clamp(viewport.X*0.86,470,680) or 680
    local height=mobile and math.clamp(viewport.Y*0.78,420,560) or 520
    local Window=WindUI:CreateWindow({
        Title="gomes hub",Author="steal an egg",Folder="GomesHub",Icon="box",Theme="Dark",Acrylic=true,Transparent=true,
        Background="rbxassetid://84152360484913",Size=UDim2.fromOffset(width,height),MinSize=Vector2.new(560,350),MaxSize=Vector2.new(850,560),
        Resizable=true,AutoScale=true,NewElements=true,BackgroundImageTransparency=0.65,HideSearchBar=false,
        ScrollBarEnabled=false,SideBarWidth=200,Topbar={Height=44,ButtonsType="Default"},
        OpenButton={Title="gomes hub",Icon="zap",CornerRadius=UDim.new(1,0),StrokeThickness=3,Enabled=true,Draggable=true,OnlyMobile=false,Scale=1,Color=ColorSequence.new(Color3.fromHex("#000000"),Color3.fromHex("#000000"))},
        User={Enabled=true,Anonymous=false,Callback=function() end},
    })
    h.gui=Window p=Window Window.IgnoreAlerts=true
    local statusObj={} function statusObj:SetTitle(...) end function statusObj:SetColor(...) end B=statusObj
    Fk=Fk or {} Fk.paraLiveDash={} function Fk.paraLiveDash:SetDesc(...) end
    updateLoading(31,"creating tabs")
    task.wait()

    local FarmTab=Window:Tab({Title="Auto Farm",Icon="box"})
    local EggTab=Window:Tab({Title="Egg Settings",Icon="egg"})
    local CharacterTab=Window:Tab({Title="Character",Icon="user"})
    local CombatTab=Window:Tab({Title="Combat",Icon="sword"})
    local AutomationTab=Window:Tab({Title="Automation",Icon="refresh-cw"})
    ...
    -- Build the Keybinds tab only after the Window and all tabs exist.
    setupKeybindSystem(KeybindsTab, Window)
    hk=FarmTab
    task.wait()
    updateLoading(38,"loading farm controls")

    local updating=false
    local teleportToggle
    pcall(function() FarmTab:Toggle({Title="Instant Pickup",Desc="Instantly pick up CarryAreaEgg prompts",Value=false,Callback=function(value) setInstantPickup(value==true) end}) end)
    task.wait()
    pcall(function() FarmTab:Toggle({Title="Auto Steal Manual",Desc="After any local prompt is completed, use the Auto Steal movement method to return directly to the Safe Area",Value=false,Callback=function(value) setManualStealReturn(value==true) end}) end)
    pcall(function() FarmTab:Toggle({Title="Automatic Steal 2",Desc="Auto Steal Manual waits 0.5s at (590, 71, -368), then continues to the Safe Area",Value=h.automaticSteal2==true,Callback=function(value) h.automaticSteal2=value==true end}) end)
    task.wait()
    pcall(function() teleportToggle=FarmTab:Toggle({Title="Auto Steal (Teleport)",Desc="Teleport to steal eggs continuously",Value=h.autoFarmLoop==true,Callback=function(value)
        if updating then return end
        if value then T4("WARP") elseif Y4=="WARP" or h.autoFarmLoop then T4("NONE") end
    end}) end)
    task.wait()
    pcall(function() FarmTab:Toggle({Title="Auto Steal",Desc="Smoothly move to eggs using the original Tween farm method",Value=h.pureTweenFarm==true,Callback=function(value)
        if updating then return end
        if value then T4("TWEEN") elseif Y4=="TWEEN" or h.pureTweenFarm then T4("NONE") end
    end}) end)
    task.wait()
    pcall(function() FarmTab:Toggle({Title="Auto Place (Every 5)",Desc="Return home every 5 steals and place the stored eggs",Value=h.autoPlaceEvery5==true,Callback=function(value) h.autoPlaceEvery5=value==true if not value then h.batchStealCount=0 end end}) end)
    task.wait()
    pcall(function() FarmTab:Toggle({Title="Auto Return",Desc="Automatically return to the safe area after stealing",Value=h.autoGlide==true,Callback=function(value) h.autoGlide=value==true end}) end)
    task.wait()
    pcall(function() FarmTab:Toggle({Title="Auto Hatch",Desc="Automatically hatch ready eggs continuously",Value=h.autoHatch==true,Callback=function(value) h.autoHatch=value==true end}) end)
    task.wait()
    pcall(function() FarmTab:Toggle({Title="Auto Treadmill",Desc="Run on the base treadmill when no target eggs are spawned",Value=false,Callback=function(value) h.autoTreadmill=value==true pcall(x) pcall(n4) if not value and (h.onTreadmill or L4()) then pcall(M4) end end}) end)
    task.wait()
    pcall(function()
        local sg=Instance.new("ScreenGui")
        sg.Name="GomesHubStealCounter"
        sg.ResetOnSpawn=false
        sg.IgnoreGuiInset=true
        sg.Parent=o:WaitForChild("PlayerGui")
        local frame=Instance.new("Frame")
        frame.Name="Counter"
        frame.Size=UDim2.fromOffset(150,36)
        frame.Position=UDim2.new(1,-166,0,12)
        frame.BackgroundColor3=Color3.fromRGB(18,18,24)
        frame.BackgroundTransparency=0.08
        frame.BorderSizePixel=0
        frame.Parent=sg
        local corner=Instance.new("UICorner") corner.CornerRadius=UDim.new(0,10) corner.Parent=frame
        local stroke=Instance.new("UIStroke") stroke.Thickness=1 stroke.Transparency=0.55 stroke.Parent=frame
        ...
        label.Parent=frame
        h.stealCounterGui=sg
        h.stealCounterLabel=label
        pcall(createSafeReturnButton)
    end)

    updateLoading(52,"loading egg settings")

    local zoneValues={"Light Dark","Titan Temple","Cherry Blossom","Cosmic","Prehistoric","Abyss Ocean","Volcano","Snow","Jungle","Desert","Lake","Forest"}
    local selectedZones={}
    for _,zone in ipairs(zoneValues) do if h.selectedZones and h.selectedZones[zone]==true then table.insert(selectedZones,zone) end end
    pcall(function() EggTab:Dropdown({Title="Selected Zones",Desc="Choose which zones can be farmed",Values=zoneValues,Value=selectedZones,Multi=true,AllowNone=true,SearchBarEnabled=true,Callback=function(value)
        local selected={}
        if type(value)=="table" then for _,item in ipairs(value) do if type(item)=="string" then selected[item]=true end end for key,item in pairs(value) do if type(key)=="string" and item==true then selected[key]=true end end elseif type(value)=="string" then selected[value]=true end
        h.selectedZones=selected pcall(x)
    end}) end)
    task.wait()
    local rarityValues={"Divine","Eternal","Secret","Cosmic","Mythic","Legendary","Epic","Rare","Uncommon","Common"}
    local selectedRarities={}
    for _,rarity in ipairs(rarityValues) do if h.selectedRarities and h.selectedRarities[rarity]==true then table.insert(selectedRarities,rarity) end end
    pcall(function() EggTab:Dropdown({Title="Selected Rarities",Desc="Choose which egg rarities can be farmed",Values=rarityValues,Value=selectedRarities,Multi=true,AllowNone=true,SearchBarEnabled=true,Callback=function(value)
        local selected={}
        if type(value)=="table" then for _,item in ipairs(value) do if type(item)=="string" then selected[item]=true end end for key,item in pairs(value) do if type(key)=="string" and item==true then selected[key]=true end end elseif type(value)=="string" then selected[value]=true end
        h.selectedRarities=selected pcall(x)
    end}) end)
    task.wait()
    pcall(function() EggTab:Toggle({Title="Always Steal Secret+",Desc="Always target Secret, Eternal and Divine eggs",Value=false,Callback=function(value) h.alwaysCollectSecretPlus=value==true pcall(x) end}) end)
    task.wait()
    updateLoading(66,"loading character controls")

    pcall(function() CharacterTab:Toggle({Title="Godmode",Desc="Enable the original desync godmode protection",Value=false,Callback=function(value) h.godmode=value==true if value then pcall(enableDesyncGodmode) else pcall(disableDesyncGodmode) end end}) end)
    task.wait()
    local speedSlider
    pcall(function() speedSlider=CharacterTab:Slider({Title="Flight Speed",Desc="Control the flight speed used by Auto Farm",Step=1,Value={Min=100,Max=1000,Default=math.clamp(tonumber(h.glideSpeed) or 600,100,1000)},IsTooltip=true,IsTextbox=true,Callback=function(value) value=math.clamp(math.floor(tonumber(value) or 600),100,1000) h.glideSpeed=value Y(value) end}) end)
    Fk.sliderSpeed=speedSlider
    task.wait()
    pcall(function() CharacterTab:Button({Title="Get Unstuck",Desc="Instantly escape from treadmill or map geometry",Icon="unlock",Callback=function() pcall(M4) pcall(C4) pcall(D4) end}) end)
    task.wait()
    updateLoading(72,"loading combat controls")
    pcall(function() CombatTab:Toggle({Title="Bat Aura",Desc="Automatically swing the bat at nearby players",Value=false,Callback=function(value) setBatAura(value==true) end}) end)
    task.wait()
    pcall(function() CombatTab:Toggle({Title="Anti Trap",Desc="Remove trap state, anchoring and forced ragdoll",Value=false,Callback=function(value) setAntiTrap(value==true) end}) end)
    task.wait()
    pcall(function() CombatTab:Toggle({Title="Anti Ragdoll",Desc="Prevent ragdoll, falling down and forced joint disabling",Value=false,Callback=function(value) setAntiRagdoll(value==true) end}) end)
    task.wait()
    updateLoading(78,"loading automation")

    pcall(function() AutomationTab:Toggle({Title="Auto Equip Best",Desc="Automatically equip the best pets every 6 seconds",Value=AutoEquipBest,Callback=function(value) setAutoEquipBest(value==true) end}) end)
    task.wait()
    pcall(function() AutomationTab:Button({Title="Equip Best Now",Desc="Equip the best pets immediately",Icon="sparkles",Callback=function() pcall(equipBestNow) end}) end)
    task.wait()
    pcall(function() AutomationTab:Toggle({Title="Auto Upgrade Treadmill",Desc="Automatically upgrade the base treadmill when affordable",Value=false,Callback=function(value) h.autoUpgradeTreadmill=value==true end}) end)
    task.wait()
    pcall(function() AutomationTab:Toggle({Title="Auto Buy & Equip Trails",Desc="Automatically purchase and equip the best speed trail available",Value=false,Callback=function(value) h.autoBuyTrails=value==true end}) end)
    task.wait()
    pcall(function() AutomationTab:Toggle({Title="Hide Not Enough Money",Desc="Hide the script's not-enough-money notifications when shopping",Value=h.hideNotEnoughMoney==true,Callback=function(value) h.hideNotEnoughMoney=value==true end}) end)
    task.wait()
    updateLoading(84,"loading server controls")
    pcall(function() ServerTab:Section({Title="Server Hop"}) end)
    task.wait()
    pcall(function() ServerTab:Button({Title="Server Hop",Desc="Join another public server",Icon="globe",Callback=function()
        local ok,reason=serverHop()
        if ok then pcall(function() WindUI:Notify({Title="gomes hub",Content="joining another server...",Duration=2}) end)
        else pcall(function() WindUI:Notify({Title="gomes hub",Content="server hop failed: "..tostring(reason),Duration=3}) end) end
    end}) end)
    task.wait()
    updateLoading(88,"loading settings")
    pcall(function() SettingsTab:Section({Title="Safe Return Button"}) end)
    task.wait()
    pcall(function() SettingsTab:Button({Title="Test Safe Return",Desc="Use the same Auto Steal movement method to return to the Safe Area",Icon="navigation",Callback=function() task.spawn(function() returnToSafeAreaManual("settings") end) end}) end)
    task.wait()
    pcall(function() SettingsTab:Section({Title="Performance & Graphics"}) end)
    task.wait()
    pcall(function() SettingsTab:Toggle({Title="Ultra Potato Mode",Desc="Reduce visual effects and graphics load for higher FPS",Value=false,Callback=function(value)
        h.performanceMode=value==true
        if value then pcall(Mk) else pcall(Ik) end
    end}) end)
    task.wait()
    pcall(function() SettingsTab:Toggle({Title="Disable 3D Rendering",Desc="Disable 3D rendering to reduce GPU usage",Value=false,Callback=function(value)
        h.disable3D=value==true
        pcall(function() y:Set3dRenderingEnabled(not value) end)
    end}) end)
    task.wait()
    pcall(function() SettingsTab:Section({Title="System"}) end)
    task.wait()
    pcall(function() SettingsTab:Toggle({Title="Anti-AFK",Desc="Keep the player from being marked idle",Value=false,Callback=function(value)
        h.antiAFK=value==true
        if value then pcall(bk) else pcall(Ak) end
    end}) end)
    task.wait()
    pcall(function() SettingsTab:Button({Title="Reset Character State",Desc="Clear velocity, cancel push and restore character movement",Icon="refresh-cw",Callback=function()
        pcall(D4) pcall(u4)
        pcall(function() WindUI:Notify({Title="gomes hub",Content="Character state reset",Duration=2}) end)
    end}) end)
    task.wait()
    pcall(function() SettingsTab:Button({Title="Unload Script",Desc="Stop background systems and close the hub",Icon="trash-2",Callback=function() pcall(aM) end}) end)
    task.wait()
    updateLoading(90,"finishing setup")

    W4=function(value)
        pcall(function()
            updating=true
            if teleportToggle then
                if teleportToggle.Set then teleportToggle:Set(value==true) elseif teleportToggle.SetValue then teleportToggle:SetValue(value==true) end
            end
            updating=false
        end)
    end
    ...
    pcall(function() CreditsTab:Section({Title="Credits"}) end)
    task.wait()
    pcall(function() CreditsTab:Paragraph({Title="Creator",Content="gomesff1"}) end)
    task.wait()
    pcall(function() CreditsTab:Paragraph({Title="Discord",Content="discord.gg/38qDq4HH"}) end)
    task.wait()
    pcall(function() CreditsTab:Button({Title="Copy Discord Invite",Desc="Copy the Gomes Hub Discord invite",Icon="copy",Callback=function()
        pcall(function() setclipboard("https://discord.gg/38qDq4HH") end)
        pcall(function() WindUI:Notify({Title="gomes hub",Content="Discord invite copied",Duration=2}) end)
    end}) end)
    task.wait()

    pcall(function() AutomationTab:Button({Title="Refresh Settings",Desc="Apply the current zone and rarity settings",Icon="refresh-cw",Callback=function() pcall(x) WindUI:Notify({Title="gomes hub",Content="Settings refreshed",Duration=2}) end}) end)
    task.wait()
    updateLoading(96,"ready")
    pcall(function() WindUI:Notify({Title="gomes hub",Content="gomes hub loaded",Duration=3}) end)
...
H("[+] Initializing gomes hub x WindUI (steal an egg)...")

local function startupStep(percent, status, fn)
    if not h.alive then return false end
    StartupStage=StartupStage+1
...
-- The runtime loops stay parked while the loading screen is visible.
-- Each stage prepares one small subsystem, then yields before continuing.
updateLoading(8,"starting")
task.wait()

-- Build the GUI during the loading phase. It stays behind the loading screen
-- until the rest of the systems are ready, so the user never sees half-built controls.
startupStep(12,"loading interface",function()
    oM()
end)

startupStep(20,"preparing player state",function()
    pcall(function() D4() end)
end)
startupStep(27,"preparing egg scanner",function()
    pcall(function() ek() end)
end)
startupStep(34,"preparing target cache",function()
    pcall(function() x() end)
end)
startupStep(41,"preparing character controller",function()
    pcall(function() C4() end)
end)
startupStep(48,"preparing humanoid system",function()
    pcall(function() A4() end)
end)
startupStep(55,"preparing movement system",function()
    if o.Character then pcall(z4,o.Character) end
end)
startupStep(62,"preparing protection",function()
    -- Prepare the protection subsystem without enabling Godmode.
    pcall(function() b4(false) end)
end)
startupStep(69,"preparing status system",function()
    pcall(function() u4() end)
end)
startupStep(76,"preparing auto steal systems",function()
    -- Controllers are already defined; keep them idle until the final gate.
    pcall(function() Y4="NONE" end)
end)
startupStep(82,"preparing automation",function()
    -- Automation loops remain parked behind StartupReady.
end)
startupStep(89,"preparing auto equip",function()
    -- Auto Equip Best is initialized but remains disabled until the user enables it.
    pcall(function() setAutoEquipBest(false) end)
end)
startupStep(94,"finalizing systems",function()
...
updateLoading(97,"activating systems")
task.wait(0.08)
StartupReady=true
updateLoading(99,"readying controls")
task.wait(0.12)
H("[+] Runtime systems loaded progressively.")
H("[+] Auto Humanoid Swap & Rigid Joint Locking Active.")
finishLoading()

o.CharacterAdded:Connect(function(character)
    task.wait(0.6)
    if h.alive then
        D4();n4();C4();A4();b4(false);if h.antiRagdoll then z4(character) end;u4()
    end
end)

H("[+] gomes hub Ready! All-In-One Built-in Anti-AFK & Prometheus Ready!")
