...
local function oM(...)
    ...
    pcall(function()
        ...
        FarmTab:Dropdown({
            Title="Selected Area",
            Desc="Choose one fixed area",
            Values={"cosmic","kitsune","titan","anjel"},
            Value={selectedTravelArea},
            AllowNone=false,
            SearchBarEnabled=false,
            Callback=function(value)
                local chosen=type(value)=="table" and value[1] or value
                chosen=tostring(chosen or ""):lower()
                if travelAreas[chosen] then
                    selectedTravelArea=chosen
                end
            end
        })
        task.wait()
        FarmTab:Button({
            Title="Go to Selected Area",
            Desc="Move to the selected fixed area using the Return to Safe movement method",
            Icon="navigation",
            Callback=function()
                task.spawn(function() goToSelectedArea() end)
            end
        })
    end)
