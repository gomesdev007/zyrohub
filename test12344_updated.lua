...
[ "autoFarmLoop" ]= false ,[ "pureTweenFarm" ]= false ,[ "manualStealReturn" ]= false ,[ "manualStealReturnConnection" ]=nil,[ "safeReturnButtonGui" ]=nil,[ "safeReturnButton" ]=nil,[ "safeReturnKeyConnection" ]=nil,[ "safeReturnKey" ]=Enum.KeyCode.RightControl,[ "safeReturnKeyName" ]="RightControl",[ "minimizeKeyConnection" ]=nil,[ "minimizeKey" ]=Enum.KeyCode.RightShift,[ "minimizeKeyName" ]="RightShift";
...
Q4=function(speed,sessionId,forceManual,...)
    ...
    h.statusText="[SafeReturn] Moving to fixed return position..."
    pcall(u4)
    if humanoid then
    ...
    while h.alive and h.isReturning and os.clock()<deadline do
        ...
        -- Obstacle awareness without enabling jumping.
        local obstacles={}
        ...
        for _,part in ipairs(obstacles) do
            if part and part:IsA("BasePart") then
                ...
                if horizontal<14 and math.abs(nextPosition.Y-part.Position.Y)<10 then
                    -- Move around the obstacle on X/Z instead of jumping.
        ...
        if lookDelta.Magnitude<0.05 then lookDelta=root.CFrame.LookVector else lookDelta=lookDelta.Unit end
        root.CFrame=CFrame.lookAt(nextPosition,nextPosition+lookDelta)
        root.AssemblyLinearVelocity=Vector3.zero
        root.AssemblyAngularVelocity=Vector3.zero

        if (nextPosition-lastPosition).Magnitude>0.05 then
            lastPosition=nextPosition
            lastProgress=os.clock()
        elseif os.clock()-lastProgress>2.6 then
    ...
    if h.alive and h.isReturning then
        root.CFrame=CFrame.new(fixedTarget)
        root.AssemblyLinearVelocity=Vector3.zero
        root.AssemblyAngularVelocity=Vector3.zero
    end
    ...
    -- Stop only the movement. Auto Steal Manual remains enabled.
    h.isReturning=false
    h.delivering=false
    h.onTreadmill=false
    h.statusText="Fixed return position reached (590, 71, -368)."
    return true
end
...
local function returnToSafeAreaManual(source)
    ...
    -- Manual Safe Return must take over the current Auto Steal movement.
    -- It does not scan/select eggs; it only reuses Q4, the existing Safe Line movement.
    h.glidingToTarget=false
    h.securingEgg=false
    h.delivering=false
    h.teleporting=false
    ...
    H("[SafeReturn] "..tostring(source or "button").." -> using Auto Steal return movement.")
...
local function setMinimizeKey(keyCode)
    if h.minimizeKeyConnection then
        pcall(function() h.minimizeKeyConnection:Disconnect() end)
        h.minimizeKeyConnection=nil
    end
    if typeof(keyCode)=="EnumItem" and keyCode.EnumType==Enum.KeyCode then
        h.minimizeKey=keyCode
        h.minimizeKeyName=keyCode.Name
    end
    if not h.alive then return end
    local ok,conn=pcall(function()
        return w.InputBegan:Connect(function(input,gameProcessed)
            if gameProcessed or not h.alive then return end
            if input.UserInputType==Enum.UserInputType.Keyboard and input.KeyCode==h.minimizeKey then
                pcall(function()
                    if h.gui and h.gui.Toggle then h.gui:Toggle() end
                end)
            end
        end)
    end)
    if ok and conn then h.minimizeKeyConnection=conn end
end
...
local function oM(...)
    ...
    task.wait()
    local minimizeKeyNames={"RightShift","RightControl","LeftShift","LeftControl","F","G","H","J","K","L","Z","X","C","V","B","N","M"}
    pcall(function() SettingsTab:Dropdown({Title="Minimize Key",Desc="Choose the keyboard key that hides or reopens the hub",Values=minimizeKeyNames,Value=h.minimizeKeyName or "RightShift",AllowNone=false,SearchBarEnabled=true,Callback=function(value)
        local name=type(value)=="table" and value[1] or value
        if type(name)=="string" and Enum.KeyCode[name] then setMinimizeKey(Enum.KeyCode[name]) end
    end}) end)
    task.wait()
    ...
    task.wait()
    pcall(setMinimizeKey, h.minimizeKey or Enum.KeyCode.RightShift)
