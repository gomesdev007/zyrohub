...
local function wk(e,r,u,w,...)
    ...
    -- Auto Steal movement speed is fixed at 355.
    local STEAL_SPEED = 355
    local V = e.Position
    V4(V, 14)
    pcall(function()
    ...
    while h.alive and h.glidingToTarget and os.clock() < deadline do
        ...
        if totalDistance <= 6 or (horizontalDistance <= 3.5 and math.abs(delta.Y) <= 6) then
            break
        end
        ...
        local targetZ = V.Z
        if xDistance > 40 then
            targetZ = H
        end

        local target = Vector3.new(V.X, V.Y, targetZ)
        local move = target - pos
        local moveUnit = move.Magnitude > 0.05 and move.Unit or Vector3.zero

        -- Kira-style directional movement: drive the Humanoid and keep the
        -- root-part velocity synchronized with the fixed travel speed.
        local velocity = moveUnit * STEAL_SPEED
        ...
        if horizontalDistance > 25 then
            ...
            for _, obstacle in ipairs(obstacles) do
                ...
                local distanceToObstacle = (nextPos - obstaclePos).Magnitude
                local dx = math.abs(nextPos.X - obstaclePos.X)
                local dz = math.abs(nextPos.Z - obstaclePos.Z)

                if distanceToObstacle < 22 or (dx < 18 and dz < 14) then
                    local safeY = obstaclePos.Y + 16
                    if target.Y < safeY then
                        target = Vector3.new(target.X, math.max(target.Y, safeY), target.Z)
                        move = target - pos
                        moveUnit = move.Magnitude > 0.05 and move.Unit or Vector3.zero
                        velocity = moveUnit * STEAL_SPEED
                    end
                    h.statusText = string.format("Gliding Out (Z: %.0f) [DODGING TRAP!]", target.Z)
                    break
                end
            end
        end
        ...
        if a then
            pcall(function()
                ...
                a.Sit = false
                a.AutoRotate = false
                a.WalkSpeed = STEAL_SPEED
                if moveUnit.Magnitude > 0.05 then
                    a:Move(moveUnit, false)
                else
                    a:Move(Vector3.zero, false)
                end
            end)
        end

        k.AssemblyLinearVelocity = Vector3.new(velocity.X, velocity.Y, velocity.Z)
        k.AssemblyAngularVelocity = Vector3.zero

        if moveUnit.Magnitude > 0.05 then
            local look = Vector3.new(moveUnit.X, 0, moveUnit.Z)
            if look.Magnitude > 0.05 then
                k.CFrame = CFrame.lookAt(k.Position, k.Position + look.Unit)
            end
        end

        h.statusText = string.format("Gliding -> Egg (%.0f studs | H: %.0f)", totalDistance, horizontalDistance)
    end

    k.CFrame = V * CFrame.new(0, 0.4, 0)
    k.AssemblyLinearVelocity = Vector3.zero
    k.AssemblyAngularVelocity = Vector3.zero
...
local function moveToFixedArea(areaName)
    if not h.alive then return false end
...
local function oM(...)
    ...
    pcall(function()
        ...
        FarmTab:Paragraph({
            ...
            Content="Cosmic, Kitsune, Titan and Anjel are available as floating buttons below Return to Safe. Their movement uses the same glide movement method."
        })
    end)
    ...
    pcall(function()
        ...
        CharacterTab:Slider({
            ...
            Desc="Controls only Cosmic, Kitsune, Titan and Anjel floating-button movement",
            Step=1,
            Value={
                Min=100,
                Max=1000,
    ...
    pcall(function()
        ...
        SettingsTab:Dropdown({
            Title="Floating Button Mode",
            Desc="single = one floating button, multiple = several floating action buttons",
            Values={"single","multiple"},
            Value={h.floatingButtonMode or "single"},
            AllowNone=false,
            SearchBarEnabled=false,
            Callback=function(value)
                local chosen=type(value)=="table" and value[1] or value
                chosen=tostring(chosen or "single"):lower()
                if chosen~="multiple" then chosen="single" end
                h.floatingButtonMode=chosen
                pcall(createSafeReturnButton)
                H("[Settings] Floating button mode: "..chosen)
            end
        })
        task.wait()
    ...
    -- Floating buttons are rebuilt from the selected Settings mode.
    pcall(createSafeReturnButton)
