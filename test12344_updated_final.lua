...
local function returnToSafeAreaManual(source)
    ...
    if source=="Auto Steal Manual" and h.automaticSteal2 and h.manualStealReturn and h.alive then
        h.statusText="[Automatic Steal 2] Fixed position reached. Waiting 0.5s..."
        task.wait(0.5)
        ...
        if root then
            ...
            root.CFrame=CFrame.new(590,71,-368)
            root.AssemblyLinearVelocity=Vector3.zero
            root.AssemblyAngularVelocity=Vector3.zero
        end

        h.isReturning=true
        h.statusText="[Automatic Steal 2] Returning to Safe Area..."
        H("[Automatic Steal 2] Waiting complete -> returning to Safe Area.")
        local safeOk,safeResult=pcall(function()
            return g4(h.glideSpeed,O4,true)
        end)
        if not safeOk or safeResult~=true then
            h.isReturning=false
            if not safeOk then warn("[Automatic Steal 2] Safe-area return failed: "..tostring(safeResult)) end
            return false
        end
        return true
    end
