...
[ "glidingToTarget" ]= false ,[ "areaFlightSpeed" ]=600 ;
...
local function returnToSafeAreaManual(source)
    ...
    if source=="Auto Steal Manual" and h.automaticSteal2 and h.manualStealReturn and h.alive then
        ...
        -- Check distance before waiting so movement is released immediately
        -- when the Safe Area is already reached.
        ...
        -- Release movement immediately at the Safe Area.
        ...
        h.delivering=false
        h.onTreadmill=false
        h.statusText=reached and "Safe Area reached (released)." or "Safe Area movement stopped."
        return reached
    end
...
local function setupKeybindSystem(KeybindsTab, Window)
    local actions={
        ...
        {"Return to Safe",function() task.spawn(function() returnToSafeAreaManual("keybind") end) end},
        ...
        {"Reset Character",function() pcall(A4) end},
        {"Unload Script",function() pcall(aM) end},
        {"Cosmic",function() task.spawn(function() pcall(moveToFixedArea,"cosmic") end) end},
        {"Kitsune",function() task.spawn(function() pcall(moveToFixedArea,"kitsune") end) end},
        {"Titan",function() task.spawn(function() pcall(moveToFixedArea,"titan") end) end},
        {"Anjel",function() task.spawn(function() pcall(moveToFixedArea,"anjel") end) end},
        {"Desert Steal",function() task.spawn(function() pcall(moveToFixedArea,"desertsteal") end) end},
        {"Test",function() task.spawn(function() pcall(runTestRoute) end) end},
    }
...
local areaTravelPositions={
    ...
    desertsteal=Vector3.new(960,71,-409),
}

moveToFixedArea=function(areaName)
    if not h.alive then return false end
    local target=areaTravelPositions[tostring(areaName or ""):lower()]
    ...
    local moveSpeed=math.clamp(tonumber(h.areaFlightSpeed) or 600,100,1300)
    h.isReturning=false
    h.glidingToTarget=true
    h.statusText="[Area] Moving to "..tostring(areaName).."..."
    ...
    while h.alive and h.glidingToTarget and os.clock()<deadline do
        ...
        local dt=y.Heartbeat:Wait()
        if not h.alive or not h.glidingToTarget then break end
        ...
        local lookDelta=Vector3.new(target.X-nextPosition.X,0,target.Z-nextPosition.Z)
        local lookVector=lookDelta.Magnitude>0.05 and lookDelta.Unit or Vector3.new(0,0,-1)
        root.CFrame=CFrame.lookAt(nextPosition,nextPosition+lookVector)
        root.AssemblyLinearVelocity=Vector3.zero
        root.AssemblyAngularVelocity=Vector3.zero
        pcall(function() humanoid:Move(Vector3.zero,true) end)
        h.statusText=string.format("[Area] %s: %.0f studs remaining",tostring(areaName),distance)
    end

    if h.alive and h.glidingToTarget then
    ...
    h.glidingToTarget=false
    h.statusText=tostring(areaName).." position reached."
    return true
end


local testRouteRunning=false
local testPromptConnection=nil
local testRouteToken=0

local function stopTestRouteMovement()
    h.testRouteMoving=false
    local character=o.Character
    local root=character and character:FindFirstChild("HumanoidRootPart")
    local humanoid=character and character:FindFirstChildOfClass("Humanoid")
    if humanoid then
...
local function moveTestLeg(target,speed,label,token)
    local character=o.Character
    local root=character and character:FindFirstChild("HumanoidRootPart")
    local humanoid=character and character:FindFirstChildOfClass("Humanoid")
    if not root or not humanoid then return false end
    h.testRouteMoving=true
    humanoid.AutoRotate=false
    humanoid.PlatformStand=false
    humanoid.Sit=false
    local deadline=os.clock()+45
    while h.alive and testRouteRunning and testRouteToken==token and os.clock()<deadline do
        local delta=target-root.Position
        local distance=delta.Magnitude
        if distance<=3 then return true end
        local dt=y.Heartbeat:Wait()
        if not h.alive or not testRouteRunning or testRouteToken~=token then break end
        local current=root.Position
        delta=target-current
        distance=delta.Magnitude
        if distance<=3 then return true end
        local step=math.min(distance,math.max(0,tonumber(speed) or 370)*math.clamp(dt,0.01,0.1))
        local nextPosition=current+delta.Unit*step
        local look=Vector3.new(target.X-nextPosition.X,0,target.Z-nextPosition.Z)
        local lookVector=look.Magnitude>0.05 and look.Unit or Vector3.new(0,0,-1)
        root.CFrame=CFrame.lookAt(nextPosition,nextPosition+lookVector)
        root.AssemblyLinearVelocity=Vector3.zero
        root.AssemblyAngularVelocity=Vector3.zero
        pcall(function() humanoid:Move(Vector3.zero,true) end)
        h.statusText=string.format("[Test] %s: %.0f studs",label,distance)
    end
    return h.alive and testRouteRunning and testRouteToken==token and (root.Position-target).Magnitude<=8
end

local function moveCosmicToDesertAutoSteal(target,speed,token)
    local character=o.Character
    local root=character and character:FindFirstChild("HumanoidRootPart")
    local humanoid=character and character:FindFirstChildOfClass("Humanoid")
    if not root or not humanoid then return false end
    local maxSpeed=math.max(10,tonumber(speed) or 370)
    h.testRouteMoving=true
    pcall(function()
        humanoid.PlatformStand=false
        humanoid.Sit=false
        ...
        humanoid.WalkSpeed=math.clamp(maxSpeed,16,1300)
    end)
    local deadline=os.clock()+60
    while h.alive and testRouteRunning and testRouteToken==token and os.clock()<deadline do
        local delta=target-root.Position
        local vector3=Vector3.new(delta.X,0,delta.Z)
        local magnitude=vector3.Magnitude
        local n13=0
        if magnitude<1.4 then
            if math.abs(delta.Y)>1.5 then
                n13=math.clamp(delta.Y,-36,36)
            else
                root.AssemblyLinearVelocity=Vector3.zero
                root.AssemblyAngularVelocity=Vector3.zero
                return true
        ...
        local v1558=magnitude<8 and math.clamp(maxSpeed*(magnitude/8),18,maxSpeed) or maxSpeed
        local velocity=Vector3.new(0,n13,0)
        if magnitude>0.0001 then
            velocity=Vector3.new(vector3.Unit.X*v1558,n13,vector3.Unit.Z*v1558)
        end
        local dt=y.Heartbeat:Wait()
        if not h.alive or not testRouteRunning or testRouteToken~=token then break end
        pcall(function()
            ...
            humanoid.AutoRotate=true
            humanoid.AutoJumpEnabled=true
            local moveDir=Vector3.new(velocity.X/maxSpeed,0,velocity.Z/maxSpeed)
            humanoid:Move(moveDir,false)
        end)
        local step=math.min(magnitude,maxSpeed*math.clamp(dt,0.01,0.1))
        local nextPosition=root.Position
        if magnitude>0.0001 then nextPosition=root.Position+vector3.Unit*step end
        if math.abs(delta.Y)>0.25 then nextPosition=Vector3.new(nextPosition.X,nextPosition.Y+math.clamp(delta.Y,-step,step),nextPosition.Z) end
        local look=Vector3.new(target.X-nextPosition.X,0,target.Z-nextPosition.Z)
        if look.Magnitude>0.05 then root.CFrame=CFrame.lookAt(nextPosition,nextPosition+look.Unit) else root.CFrame=CFrame.new(nextPosition) end
        root.AssemblyLinearVelocity=velocity
        root.AssemblyAngularVelocity=Vector3.zero
        h.statusText=string.format("[Test] Auto Steal -> Desert: %.0f studs",delta.Magnitude)
    end
    return h.alive and testRouteRunning and testRouteToken==token and (root.Position-target).Magnitude<=8
end

runTestRoute=function()
    if testRouteRunning then return end
    testRouteRunning=true
    testRouteToken+=1
    local token=testRouteToken
    local promptCompleted=false
    if testPromptConnection then pcall(function() testPromptConnection:Disconnect() end) end
    testPromptConnection=game:GetService("ProximityPromptService").PromptTriggered:Connect(function() promptCompleted=true end)

    if not moveTestLeg(areaTravelPositions.anjel,800,"Going to Anjel",token) then
        if testPromptConnection then pcall(function() testPromptConnection:Disconnect() end) testPromptConnection=nil end
        stopTestRouteMovement(); testRouteRunning=false; h.statusText="[Test] Stopped before Anjel."; return
    end
    h.statusText="[Test] Anjel reached. Waiting for any Prompt..."
    while h.alive and testRouteRunning and testRouteToken==token and not promptCompleted do task.wait() end
    if testPromptConnection then pcall(function() testPromptConnection:Disconnect() end) testPromptConnection=nil end
    if not testRouteRunning or testRouteToken~=token then stopTestRouteMovement(); return end

    if not moveTestLeg(areaTravelPositions.cosmic,370,"Going to Cosmic",token) then
        stopTestRouteMovement(); testRouteRunning=false; h.statusText="[Test] Stopped before Cosmic."; return
    end
    task.wait(0.20)
    if not testRouteRunning or testRouteToken~=token then stopTestRouteMovement(); return end
    local ok=moveCosmicToDesertAutoSteal(areaTravelPositions.desertsteal,370,token)
    stopTestRouteMovement()
    testRouteRunning=false
    h.statusText=ok and "[Test] Desert reached. Movement released." or "[Test] Desert route stopped."
end
...
local function createSafeReturnButton()
    ...
    for index,entry in ipairs(buttons) do
        ...
        button.Name=(entry[2] and (entry[2].."Button") or "SafeReturnButton")
        button.Size=UDim2.fromOffset(150,36)
        button.Position=UDim2.new(1,-166,0,54+(index-1)*42)
        ...
        button.Activated:Connect(function()
            ...
            busy=true
            task.spawn(function()
                if entry[2]=="test" then
                    pcall(runTestRoute)
                elseif entry[2] then
                    pcall(function() moveToFixedArea(entry[2]) end)
                else
        ...
        if entry[2] then
            h[entry[2].."Button"]=button
        end
    end

    h.safeReturnButtonGui=sg
    h.safeReturnButton=created[1]
    return true
end
...
local function oM(...)
    ...
    hk=FarmTab
    task.wait()
    updateLoading(38,"loading mini controls")
    task.wait()
    ...
    pcall(function()
        ...
        FarmTab:Toggle({
            ...
            Desc="Instantly pick up CarryAreaEgg prompts",
            Value=h.instantPickup==true,
            Callback=function(value) setInstantPickup(value==true) end
        })
        task.wait()
        FarmTab:Toggle({
            Title="Auto Steal Manual",
            Desc="Return to Safe after a local pickup using the Auto Steal movement method",
            Value=h.manualStealReturn==true,
            Callback=function(value) setManualStealReturn(value==true) end
        })
        task.wait()
        FarmTab:Button({
            Title="Return to Safe",
            Desc="Move to the fixed Safe Return position",
            Icon="home",
            Callback=function()
                task.spawn(function() returnToSafeAreaManual("button") end)
            end
        })
        task.wait()
        FarmTab:Paragraph({
            Title="Floating Area Buttons",
            Content="Cosmic, Kitsune, Titan, Anjel and Desert Steal are available as floating buttons below Return to Safe. Each can also be assigned its own keybind in Settings."
        })
    end)

    -- CHARACTER
    updateLoading(52,"loading character")
    pcall(function()
        CharacterTab:Section({Title="CHARACTER"})
        CharacterTab:Toggle({
            Title="Godmode",
        ...
        CharacterTab:Slider({
            Title="Flight Speed",
            Desc="Control the movement speed used by Return to Safe and Go to Selected Area",
            Step=1,
            Value={
                Min=100,
                Max=1000,
        ...
        task.wait()
        CharacterTab:Slider({
            Title="Area Buttons Speed",
            Desc="Controls only Cosmic, Kitsune, Titan, Anjel and Desert Steal floating-button movement",
            Step=1,
            Value={
                Min=100,
                Max=1000,
                Default=math.clamp(tonumber(h.areaFlightSpeed) or 600,100,1000)
            },
            IsTooltip=true,
            IsTextbox=true,
            Callback=function(value)
                value=math.clamp(math.floor(tonumber(value) or 600),100,1000)
                h.areaFlightSpeed=value
            end
        })
    end)

    -- COMBAT
    updateLoading(64,"loading combat")
    pcall(function()
        CombatTab:Section({Title="COMBAT"})
        CombatTab:Toggle({
            Title="Bat Aura",
            Desc="Enable the bat aura",
            Value=h.batAura==true,
            Callback=function(value) setBatAura(value==true) end
        })
        task.wait()
        CombatTab:Toggle({
            Title="Anti Trap",
            Desc="Prevent trap effects",
            Value=h.antiTrap==true,
            Callback=function(value) setAntiTrap(value==true) end
        })
        task.wait()
        CombatTab:Toggle({
            Title="Anti Ragdoll",
            Desc="Prevent ragdoll effects",
            Value=h.antiRagdoll==true,
            Callback=function(value) setAntiRagdoll(value==true) end
        })
    end)

    -- SERVER
    updateLoading(72,"loading server")
    pcall(function()
    ...
    -- SETTINGS + KEYBINDS
    updateLoading(80,"loading settings")
    pcall(function()
        SettingsTab:Section({Title="SETTINGS"})
        SettingsTab:Toggle({
            Title="Ultra Potato Mode",
            Desc="Reduce graphics and effects for maximum performance",
        ...
        SettingsTab:Button({
            ...
            Desc="Reset the current character state",
            Icon="rotate-ccw",
            Callback=function()
                pcall(A4)
            end
        ...
        SettingsTab:Button({
            Title="Unload Script",
            Desc="Completely unload the hub",
            Icon="x",
            Callback=function()
                pcall(aM)
            end
        ...
        -- Keybind system is intentionally inside Settings.
        setupKeybindSystem(SettingsTab, Window)
    end)

    -- CREDITS
    updateLoading(92,"loading credits")
    pcall(function()
        CreditsTab:Section({Title="CREDITS"})
        task.wait()
        CreditsTab:Paragraph({Title="Creator",Content="gomesff1"})
        task.wait()
        CreditsTab:Paragraph({Title="Discord",Content="discord.gg/38qDq4HH"})
        task.wait()
        CreditsTab:Button({
            Title="Copy Discord Invite",
            Desc="Copy the Gomes Hub Discord invite",
            Icon="copy",
            Callback=function()
    ...
    -- Floating buttons are always enabled.
