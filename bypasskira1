...
Q4=function(speed,sessionId,forceManual,...)
    ...
    local fixedTarget=Vector3.new(523,71,-364)
...
local function returnToSafeAreaManual(source)
    ...
    H("[SafeReturn] "..tostring(source or "button").." -> fixed position (523, 71, -364).")
...
moveToFixedArea=function(areaName)
    ...
    local oldPureTween=h.pureTweenFarm
    local oldAutoFarm=h.autoFarmLoop
    local oldTeleporting=h.teleporting

    -- Usa exatamente o mesmo motor de movimento do AutoSteal (R4 -> wk).
    -- pureTweenFarm fica temporariamente ativo somente para que o guard do
    -- próprio wk permita o deslocamento quando o botão é usado fora do farm.
    ...
    local success, result=pcall(function()
        return R4(CFrame.new(target),tonumber(h.areaFlightSpeed) or 600,nil,nil)
    end)
    if success then ok=result==true end

    h.glidingToTarget=false
    h.pureTweenFarm=oldPureTween
    h.autoFarmLoop=oldAutoFarm
    h.teleporting=oldTeleporting
    ...
    if ok then
        ...
        h.statusText=tostring(areaName).." movement stopped."
    end
    return ok
end
...
local function moveTestLeg(target,speed,label,token)
    if not h.alive or not testRouteRunning or testRouteToken~=token then return false end
    ...
    h.statusText="[Test] "..label

    local ok=false
    local success, result=pcall(function()
        return R4(CFrame.new(target),tonumber(speed) or 370,nil,nil)
    end)
    if success then ok=result==true end
    ...
    return ok and h.alive and testRouteRunning and testRouteToken==token
end
...
local function moveCosmicToDesertAutoSteal(target,speed,token)
    ...
    h.glidingToTarget=true
    h.testRouteMoving=true
    h.statusText="[Test] Auto Steal -> Desert"

    -- Mesmo R4/wk do AutoSteal original, sem trocar por MoveTo, TweenService
    -- ou outro motor de movimento.
    local ok=false
    local success, result=pcall(function()
        return R4(CFrame.new(target),tonumber(speed) or 370,nil,nil)
    end)
