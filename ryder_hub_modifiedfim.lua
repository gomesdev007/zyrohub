...
main.Size = UDim2.fromOffset(255,470)
main.Position = UDim2.new(.5,-127,.5,-235)
...
--==================================================
-- AUTO SLAP TP CARD (CUSTOMIZADO)
--==================================================

local slapTPCard = Instance.new("Frame")
slapTPCard.Position = UDim2.fromOffset(16,140)
...
slapTPToggle.MouseButton1Click:Connect(function()
    ...
    if slapTPEnabled then
        ...
        -- Abre dropdown se não tiver player selecionado
...
--==================================================
-- AUTO SLAP TP DROPDOWN (INVISIBLE BY DEFAULT)
--==================================================

local slapTPDropdown = Instance.new("Frame")
slapTPDropdown.Position = UDim2.fromOffset(16,140)
slapTPDropdown.Size = UDim2.fromOffset(223,0)
slapTPDropdown.BackgroundColor3 = Color3.fromRGB(17,17,20)
slapTPDropdown.BorderSizePixel = 0
slapTPDropdown.Visible = false
slapTPDropdown.Parent = main
slapTPDropdown.ZIndex = 10

local dropdownCorner = Instance.new("UICorner")
dropdownCorner.CornerRadius = UDim.new(0,12)
dropdownCorner.Parent = slapTPDropdown

local dropdownStroke = Instance.new("UIStroke")
dropdownStroke.Color = Color3.fromRGB(34,34,38)
dropdownStroke.Thickness = 1
dropdownStroke.Parent = slapTPDropdown
...
dropdownScroll.Parent = slapTPDropdown
...
resetButton.Text = "Reset Auto Slap"
...
resetButton.MouseButton1Click:Connect(function()
    ...
    -- Abre o dropdown para selecionar novamente
    task.wait(0.25)
    showDropdown()
end)
...
footer.Position = UDim2.fromOffset(15,446)
...
local function populateDropdown()
    ...
    for _, child in pairs(dropdownScroll:GetChildren()) do
        if child:IsA("GuiObject") and child ~= dropdownLayout then
            child:Destroy()
        end
    end

    local function createDropdownButton(targetPlayer)
    ...
    for _, targetPlayer in pairs(Players:GetPlayers()) do
        if targetPlayer ~= player then
            createDropdownButton(targetPlayer)
        end
    end

    -- Atualizar tamanho do scroll
    dropdownScroll.CanvasSize = UDim2.fromOffset(0, dropdownLayout.AbsoluteContentSize.Y + 6)
...
local function showDropdown()
    slapTPDropdown.Visible = true
    slapTPDropdown.Size = UDim2.fromOffset(223,0)
    populateDropdown()
    
    -- Calcular altura baseado no número de players
    local playerCount = 0
    ...
    local targetHeight = math.min(playerCount * 31 + 3, 150) -- Max 150 pixels
    
    TweenService:Create(
        slapTPDropdown,
        TweenInfo.new(.25,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
        {Size = UDim2.fromOffset(223, targetHeight)}
    ):Play()
end
