...
local function resolveTravelPosition(name)
    name=tostring(name or ""):lower()
    local aliases={
        cosmic={"cosmic"},
        kitsune={"kitsune"},
        titan={"titan","titan temple"},
        anjel={"anjel","angel"},
    }
    local wanted=aliases[name] or {name}
    local best=nil
    local function scoreText(text)
        text=tostring(text or ""):lower()
        local score=0
        for _,needle in ipairs(wanted) do
            if text==needle then score=1000
            elseif string.find(text,needle,1,true) then score=500 end
        end
        return score
    end
    for _,obj in ipairs(r:GetDescendants()) do
        local score=scoreText(obj.Name)
        if score>0 then
            local pos
            ...
            elseif obj:IsA("Model") then
                ...
                if pp then pos=pp.Position else pos=obj:GetPivot().Position end
            elseif obj:IsA("Attachment") then
                pos=obj.WorldPosition
            end
            if pos then
                if not best or score>best.score then best={position=pos,score=score} end
            end
        end
    end
...
local function moveManualToPosition(target,source)
    if not h.alive or typeof(target)~="Vector3" then return false end
    local character=o.Character
    local root=character and character:FindFirstChild("HumanoidRootPart")
    local humanoid=character and character:FindFirstChildOfClass("Humanoid")
    if not root then return false end
    ...
    h.stateTime=os.clock()
    h.statusText="[Multi] Moving to "..tostring(source or "target").."..."
    if humanoid then
        humanoid.AutoRotate=false
        humanoid.PlatformStand=false
        humanoid.Sit=false
    ...
    root.AssemblyLinearVelocity=Vector3.zero
    root.AssemblyAngularVelocity=Vector3.zero
    local speed=math.max(60,tonumber(h.glideSpeed) or 350)
    local deadline=os.clock()+30

    while h.alive and h.isReturning and h.glidingToTarget and os.clock()<deadline do
        local current=root.Position
        local delta=target-current
        local distance=delta.Magnitude
        if distance<=6 then break end
        local dt=y.Heartbeat:Wait()
        if not h.alive or not h.isReturning or not h.glidingToTarget then break end
        current=root.Position
        delta=target-current
        distance=delta.Magnitude
        if distance<=6 then break end

        local step=math.min(distance,speed*math.clamp(dt,0.01,0.1))
        local nextPosition=current+delta.Unit*step
        local horizontal=Vector2.new(nextPosition.X,nextPosition.Z)
        local targetHorizontal=Vector2.new(target.X,target.Z)

        if (horizontal-targetHorizontal).Magnitude>25 then
            local obstacles={}
            pcall(function() obstacles=i4() or {} end)
            for _,part in ipairs(obstacles) do
                if part and part:IsA("BasePart") then
                    local u=part.Position
                    local w=(Vector3.new(nextPosition.X,0,nextPosition.Z)-Vector3.new(u.X,0,u.Z)).Magnitude
                    if w<22 and math.abs(nextPosition.Y-u.Y)<10 then
                        local side=Vector3.new(-(nextPosition.Z-u.Z),0,nextPosition.X-u.X)
                        if side.Magnitude>0.05 then nextPosition=nextPosition+side.Unit*12 end
                        break
                    end
                end
            end
        ...
        root.AssemblyLinearVelocity=Vector3.zero
        root.AssemblyAngularVelocity=Vector3.zero
        if humanoid then pcall(function() humanoid:Move(Vector3.zero,true) end) end
        h.statusText=string.format("[Multi] %s: %.0f studs",tostring(source or "Target"),distance)
    end

    local reached=(root.Position-target).Magnitude<=8
    root.AssemblyLinearVelocity=Vector3.zero
    root.AssemblyAngularVelocity=Vector3.zero
    if humanoid then
        pcall(function() humanoid:Move(Vector3.zero,true) end)
    ...
    h.glidingToTarget=false
    h.isReturning=false
    h.statusText=reached and (tostring(source or "Target").." reached") or (tostring(source or "Target").." movement stopped")
    return reached
end

local function goToMultiArea(name)
    local position=resolveTravelPosition(name)
    if not position then
        pcall(function() WindUI:Notify({Title="Multi",Content="Position not found: "..tostring(name),Duration=3}) end)
        return false
    end
    return moveManualToPosition(position,name)
end
...
local function createSafeReturnButton()
    ...
    local buttons={
        {"return to safe",function() returnToSafeAreaManual("button") end},
        {"cosmic",function() goToMultiArea("cosmic") end},
        {"kitsune",function() goToMultiArea("kitsune") end},
        {"titan",function() goToMultiArea("titan") end},
        {"anjel",function() goToMultiArea("anjel") end},
    }
    local created={}
    for index,entry in ipairs(buttons) do
        local button=Instance.new("TextButton")
        button.Name=entry[1]:gsub("%s+","").."Button"
        ...
        created[entry[1]]=button
    end
    h.safeReturnButtonGui=sg
    h.safeReturnButton=created["return to safe"]
    return true
end
...
local function oM(...)
    ...
    pcall(function()
        ...
        SettingsTab:Dropdown({
            ...
            Desc="multiple = floating action buttons displayed one below another",
            Values={"multiple"},
            Value={"multiple"},
            AllowNone=false,
            SearchBarEnabled=false,
            Callback=function()
                h.floatingButtonMode="multiple"
                pcall(createSafeReturnButton)
                H("[Settings] Floating button mode: multiple")
            end
        })
        task.wait()
